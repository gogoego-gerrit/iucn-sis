...
public class TaxomaticAssessmentMover extends LayoutContainer {
	...
	protected final TaxonNode nodeToMoveAssessmentsOutOf;
	...
	public TaxomaticAssessmentMover(TaxonNode currentNode) {
	...
	protected void addItem(TaxonNode nodeToMoveAssementsTo) {
	...
	protected VerticalPanel getRightSide() {
		...
		for (String assessmentID : nodeToMoveAssessmentsOutOf.getAssessments()) {
			DataListItem item = new DataListItem();
			AssessmentData data = AssessmentCache.impl.getPublishedAssessment(assessmentID, false);
			...
			protected void addViewButtonToFootprint() {
				footprintPanel.add(new QuickButton("Add", new SelectionListener<ButtonEvent>() {
					...
					public void componentSelected(ButtonEvent ce) {
						...
						addItem(TaxonomyCache.impl.getNode(display));
					}
				}));
			}
		};

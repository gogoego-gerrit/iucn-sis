...
public class SplitNodePanel extends LayoutContainer {
	...
	private LayoutContainer getLeftSide() {
		...
		TaxonomyCache.impl.getChildrenOfNode(currentNode.getId() + "", new GenericCallback<List<TaxonNode>>() {
			public void onFailure(Throwable caught) {
				WindowUtils.errorAlert("Error", "Could not fetch children, please try again later.");
			}

			public void onSuccess(List<TaxonNode> result) {
				Iterator it = ((ArrayList) result).listIterator();
				while (it.hasNext()) {
					TaxonNode next = (TaxonNode) it.next();
		...
		listBox.addChangeListener(new ChangeListener() {
			public void onChange(Widget sender) {
				...
				while (iterator.hasNext()) {
					TaxonNode cur = (TaxonNode) iterator.next();
			...
			public void componentSelected(ButtonEvent ce) {
				final Listener listener = new Listener() {
					public void handleEvent(BaseEvent be) {
						...
						TaxonNode newNode = (TaxonNode) be.getSource();
				...
				TaxonomyCache.impl.fetchNode(currentNode.getParentId(), false, new GenericCallback<TaxonNode>() {
					public void onFailure(Throwable caught) {
	...
	public void onClose() {
		...
		if (currentNode.getLevel() < TaxonNode.SPECIES) {

...
public class FuzzyExpImpl {
	...
	private ExpertResult calculateResult(Range cr, Range en, Range vu, AssessmentData assessment) {
	...
	private Range createRangeFromAssessment(AssessmentData assessment, String factor) {
		ArrayList factorInfo = (ArrayList) assessment.getFieldData(factor);
		Range result = null;
		if (factorInfo != null) {
			String tempFactor = (String) factorInfo.get(primaryField);
			if ((tempFactor != null) && (!tempFactor.trim().equalsIgnoreCase(""))) {
				result = new Range(tempFactor);
			}
		}
		return result;
	}

	private String createStringFromAssessment(AssessmentData assessment, String factor) {
		ArrayList factorInfo = (ArrayList) assessment.getFieldData(factor);

		String result = "";
		if (factorInfo != null) {
			SysDebugger.getInstance().println("this is the multiselect information " + factorInfo.get(primaryField));
			result = (String) factorInfo.get(primaryField);
		}
		return result;
	}
	 ...
	 * @return
	 */
	public ExpertResult doAnalysis(AssessmentData assessment) {
	...
	private AnalysisResult doAnalysisB(final AssessmentData assessment) {
		AnalysisResult analysis = new AnalysisResult();
	...
	private AnalysisResult doAnalysisC(final AssessmentData assessment) {
		AnalysisResult analysis = new AnalysisResult();
	...
	private AnalysisResult doAnalysisD(final AssessmentData assessment) {
		AnalysisResult analysis = new AnalysisResult();
	...
	private AnalysisResult doAnalysisE(final AssessmentData assessment) {
		AnalysisResult analysis = new AnalysisResult();

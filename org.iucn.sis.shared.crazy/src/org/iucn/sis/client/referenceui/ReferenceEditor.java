...
public class ReferenceEditor extends Window {
	...
	private AssessmentData openedUsing;
	 ...
	 *            the reference.
	 */
	public ReferenceEditor(final ReferenceUI reference) {
	...
	public ReferenceEditor(final ReferenceUI reference, final boolean canEdit) {
		super();
		setClosable(true);
		setSize(600, 400);
		...
		typesDoc.get("/refsvr/types", new GenericCallback<String>() {
			...
			public void onSuccess(String result) {
				...
				for (int i = 0; i < types.getLength(); i++) {
					...
					if (current.getNodeName().equalsIgnoreCase("type")) {
						...
						if (reference != null && reference.getReferenceType().equalsIgnoreCase(value)) {
	...
	private void addField(String label, String name, String value, int row, FlexTable table) {
		...
		if (name.equalsIgnoreCase("citation")) {
			...
			citationComplete = ReferenceUI.isCitationValid(value);
	...
	public void onDelete() {
		...
		ndoc.delete("/refsvr/reference/" + reference.getReferenceID(), new GenericCallback<String>() {
			public void onFailure(Throwable caught) {
	 ...
	 * 
	 */
	public void onSaveSuccessful(ReferenceUI returnedRef) {
		if (reference != null)
	...
	private void putFieldsIntoReference() {
		reference.setReferenceType(typeChooser.getSelectedText());
		...
		reference.addField("citation", citationHTML.getHTML());
		reference.setCitationValid(citationComplete);
	}

	protected void save() {
		reference.addField("citation", citation);
		...
		document.post("/refsvr/submit", xml, new GenericCallback<String>() {
			public void onFailure(Throwable caught) {
				WindowUtils.errorAlert("Failure", "Save Failed Unexpectedly.");
			}

			public void onSuccess(String result) {
				final ReferenceUI returnedRef = new ReferenceUI(document.getDocumentElement().getElementByTagName(
						"reference"));
	...
	private void updateFormArea(String type) {
		formArea.removeAll();
		final NativeDocument document = SimpleSISClient.getHttpBasicNativeDocument();
		document.get("/refsvr/type/" + type, new GenericCallback<String>() {
			public void onFailure(Throwable caught) {

...
public class PermissionGroupEditor extends LayoutContainer {
	...
	private String saveDataToGroup(PermissionGroup group) {
		String scopeURI = "";
		PermissionSet defaultSet = defaultPermissionSet.getSet();
		...
		if( scopeChoices.getSelectedIndex() == 1 ) {
			if( scopeData == null )
				...
				TaxonNode scopeTaxon = (TaxonNode)scopeData;
		...
		if( defaultSet != null )
			group.addPermissionResource(PermissionGroup.DEFAULT_PERMISSION_URI, defaultSet);
		else
			group.removeResource(PermissionGroup.DEFAULT_PERMISSION_URI);
		
		group.clearInheritences();
		for( BaseModelData cur : inheritenceUI.getUsedStore().getModels() )
			group.addInheritence(AuthorizationCache.impl.getGroups().get((String)cur.get("name")));
	...
	private void updateContent() {
		...
		PermissionResource defResource = group.getPreprocessedResources().get(PermissionGroup.DEFAULT_PERMISSION_URI);
		if( defResource != null && defResource.getSet() != null )
			defaultPermissionSet.setPermissionSet(defResource.getSet());
		else
			defaultPermissionSet.setPermissionSet(new PermissionSet());
		
		ArrayList<String> inherits = new ArrayList<String>();
		for( PermissionGroup cur : group.getInherits() )
			inherits.add(cur.getName());
		
		String scopeURI = group.getOriginalScopeURI(); 
		if( scopeURI == null ) {
			scope.setValue("");
	...
	private void rebuildResourceSection() {
		...
		add.addSelectionListener(new SelectionListener<ButtonEvent>() {
			public void componentSelected(ButtonEvent ce) {
				if( !group.getPreprocessedResources().containsKey("new") ) {
					PermissionResource resource = new PermissionResource("new", new PermissionSet());
		...
		for( Entry<String, PermissionResource> cur : group.getPreprocessedResources().entrySet() ) {
	...
	private PermissionResourceUI wrapInResourceUI(PermissionResource resource) {
	...
	public void updateScope(TaxonNode taxon) {
		scope.setValue(taxon.getFullName() + (taxon.getFootprint().length > 0 ? ("/" + taxon.getFootprint()[0]) : "") );
		scope.setData("scope", taxon);
	}
	
	public void updateScope(ArrayList<WorkingSetData> sets) {
		String val = "";
		String ids = "";
		for( WorkingSetData ws : sets ) {

...
import org.iucn.sis.shared.data.FindResultCache;
import org.iucn.sis.shared.data.FindResultData;
...
//	private String ids;
	...
	private void afterSkipReplace() {
		...
		if (FindResultCache.impl.doneWithCurrent()) {
			final FindResultData currentData = FindResultCache.impl.getCurrentFindResultData();
			if (currentData != null) {
				...
				if (currentData.needsReplacing()) {
					FindResultCache.impl.replace(currentData, new GenericCallback<String>() {
						...
						public void onFailure(Throwable arg0) {
							WindowUtils.errorAlert("Error", "Error making replacements in " + currentData.getName()
			...
			updateReplacePanelInfo(FindResultCache.impl.getCurrentFindResultData().getCurrentDescription(),
					FindResultCache.impl.getCurrentFindResultData().getCurrentSentence().getOriginalString(),
					FindResultCache.impl.getCurrentFindResultData().getCurrentSentence().getCurrentText(),
					FindResultCache.impl.getCurrentFindResultData().getCurrentSentenceIndex(), FindResultCache.impl
							.getCurrentFindResultData().getCurrentSentence().getOldWordIndex());
		}
	...
	private void buildReplacePanel() {
		if (results.size() > 0) {
			FindResultData data = (FindResultData) results.get(0);
			FindResultCache.impl.setCurrentFindResult(data);
			setReplacePanelInfo(data.getName(), data.getType(), data.getCurrentSentence().getOriginalString(), data
					.getCurrentSentence().getCurrentText(), data.getNumberOfResults(), data.getCurrentDescription(),
					data.getCurrentSentenceIndex(), data.getCurrentSentence().getOldWordIndex());
		} else {
			clearReplacePanel();
		}
	}
	...
	private void buildRow(FindResultData data) {

		HTML name = new HTML(data.getName());
		HTML type = new HTML(data.getType());
...
//		fileBox = new ListBox(false);
//		fileBox.addItem("");
//		// fileBox.addItem("Taxon/Taxa", FindResultCache.TAXON + "");
//		fileBox.addItem("Draft Assessments", FindResultCache.DRAFTASSESSMENT + "");
//		//if (SISPermissionChecker.hasRight(SimpleSISClient.currentUser, Roles.EDIT_PUBLISHED_ASSESSMENTS, null)) {
//			fileBox.addItem("Published Assessments", FindResultCache.PUBLISHEDASSESSMENT + "");
//			fileBox.addItem("Both types of Assessments", FindResultCache.ALLASSESSMENTS + "");
//		//}
//
//		html = new HTML("Type of Assessments to search: ");
		filesHP = new VerticalPanel();
//		filesHP.add(html);
//		filesHP.add(fileBox);
//		fileBox.setWidth("100%");
//		fileBox.addChangeListener(new ChangeListener() {
//			public void onChange(Widget arg0) {
//				fileSelectionChanged();
//			}
//		});
//		filesHP.setCellHorizontalAlignment(html, HasHorizontalAlignment.ALIGN_LEFT);
//
		...
		whereToSearchWithFileBox.addItem(CanonicalNames.RedListAssessors, CanonicalNames.RedListAssessors);
		whereToSearchWithFileBox.addItem(CanonicalNames.RedListEvaluators, CanonicalNames.RedListEvaluators);
	...
	private void setAssessmentFilter() {
		...
		WorkingSetData ws = WorkingSetCache.impl.getWorkingSet(value);
		if (ws != null) {
...
//			determineSearchType();
	...
	private void setSelectorBox() {
		selectorBox.clear();
		selectorBox.addItem("");
		for (Entry<String, WorkingSetData> ws : WorkingSetCache.impl.getWorkingSets().entrySet()) {
			String id = ws.getValue().getId();
...
//				selectorBox.addItem("All taxa in " + ((WorkingSetData) workingSets.get(key)).getWorkingSetName()
//						+ " working set", ((WorkingSetData) workingSets.get(key)).getId());
//			}
//
//		} else if (fileBox.getValue(fileBox.getSelectedIndex()).equalsIgnoreCase(FindResultCache.DRAFTASSESSMENT + "")) {
//
//			if (TaxonomyCache.impl.getCurrentNode() != null) {
//				List<AssessmentData> list = AssessmentCache.impl.getDraftAssessmentsForTaxon(TaxonomyCache.impl.getCurrentNode());
...
//						+ ((WorkingSetData) workingSets.get(key)).getWorkingSetName() + " working set", "id,"
//						+ ((WorkingSetData) workingSets.get(key)).getId());
//			}
//
...
//					AssessmentData data = (AssessmentData) AssessmentCache.impl.getPublishedAssessment((String) list
//							.get(i), false);
//					if (data != null) {
...
//			while (iter.hasNext()) {
//				String key = (String) iter.next();
//				selectorBox.addItem("All assessments in " + ((WorkingSetData) workingSets.get(key)).getWorkingSetName()
//						+ " working set", "id," + ((WorkingSetData) workingSets.get(key)).getId());
//			}
//		}
	...
	private void updateResults() {
		if (results.size() < NUMBEROFRESULTSSHOWED) {
			for (int i = results.size(); i < NUMBEROFRESULTSSHOWED; i++) {
				...
				FindResultData data = FindResultCache.impl.getFindResultData(i);

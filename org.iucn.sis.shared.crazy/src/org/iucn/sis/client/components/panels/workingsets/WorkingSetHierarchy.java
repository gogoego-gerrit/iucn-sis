		...
		binder.addSelectionChangedListener(new SelectionChangedListener<WSModel>() {
			public void selectionChanged(SelectionChangedEvent<WSModel> se) {
				...
				if (wsModel != null) {
					...
					} else if( !WorkingSetCache.impl.getCurrentWorkingSet().getId().equals(wsModel.getID())) {
						WorkingSetCache.impl.setCurrentWorkingSet(wsModel.getID());
						panelManager.workingSetBrowser.refresh();
					}
				}
...
//			final LayoutContainer listContainer = new LayoutContainer(new FillLayout());
//			listContainer.setScrollMode(Scroll.AUTO);
//			listContainer.add(tree);
	...
	public String getCurrentlySelectedWorkingSetID() {
	...
	public void setCurrentlySelected(String workingSetID) {

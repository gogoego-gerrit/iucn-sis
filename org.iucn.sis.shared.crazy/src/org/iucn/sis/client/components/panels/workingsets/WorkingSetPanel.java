...
public class WorkingSetPanel extends RefreshPortlet {
	private ArrayList<WorkingSetData> workingSets = null;
	private boolean isBuilt;
	private LayoutContainer content = null;
	private HashMap<String, String> nameToID = null;
	...
	private void createPopup(final WorkingSetData ws) {
		...
		if (ws.getSpeciesIDs().size() > 0) {
			WorkingSetCache.impl.fetchTaxaForWorkingSet(ws, new GenericCallback<String>() {
				public void onFailure(Throwable caught) {
					...
					table.setHTML(0, 1, ws.getCreator());
					table.setHTML(1, 0, "<b>Date: </b>");
					table.setHTML(1, 1, ws.getDate());
				...
				public void onSuccess(String arg0) {
					...
					for (int i = 0; i < ws.getSpeciesIDs().size(); i++) {
						species += TaxonomyCache.impl.getNode(ws.getSpeciesIDs().get(i)).getFullName() + ", ";
					}
					if (species.length() > 0) {
	...
	public void refresh() {
		if (isBuilt) {
			...
			if (workingSets.size() > 0) {
				...
				for (final WorkingSetData data : workingSets) {

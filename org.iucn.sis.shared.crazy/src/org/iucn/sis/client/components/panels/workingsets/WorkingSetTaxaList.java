...
public class WorkingSetTaxaList extends RefreshLayoutContainer {
	...
	private WorkingSetData recentWorkingSet = null;
	...
	public void deselectAll() {
		for (int i = 0; i < taxaList.getItemCount(); i++) {
			...
			if (item.isChecked()) {
				...
				CheckChangedEvent<TaxaData> event = new CheckChangedEvent<TaxaData>(null, (TaxaData) item.getData(DATA_KEY));
				if (checkListener != null)
		...
		if (recentWorkingSet.getSpeciesIDs().size() > 0) {
			WorkingSetCache.impl.fetchTaxaForWorkingSet(recentWorkingSet, new GenericCallback<String>() {
				...
				public void onSuccess(String arg0) {
					...
					for (String taxaID : recentWorkingSet.getSpeciesIDs()) {
						TaxonNode node = TaxonomyCache.impl.getNode(taxaID);
						String family = node.getFootprint()[TaxonNode.FAMILY];
						String kingdom = node.getFootprint()[TaxonNode.KINGDOM];
		...
		if (recentWorkingSet.getSpeciesIDs().size() > 0) {
			WorkingSetCache.impl.fetchTaxaForWorkingSet(recentWorkingSet, new GenericCallback<String>() {
				...
				public void onSuccess(String arg0) {
					...
					for (String taxaID : recentWorkingSet.getSpeciesIDs()) {
						TaxonNode node = TaxonomyCache.impl.getNode(taxaID);
						String genus = node.getFootprint()[TaxonNode.GENUS];
						String kingdom = node.getFootprint()[TaxonNode.KINGDOM];
		...
		if (recentWorkingSet.getSpeciesIDs().size() > 0) {
			WorkingSetCache.impl.fetchTaxaForWorkingSet(recentWorkingSet, new GenericCallback<String>() {
				...
				public void onSuccess(String arg0) {
					...
					for (String taxaID : recentWorkingSet.getSpeciesIDs()) {
						TaxonNode node = TaxonomyCache.impl.getNode(taxaID);
						String order = node.getFootprint()[TaxonNode.ORDER];
						String kingdom = node.getFootprint()[TaxonNode.KINGDOM];
			...
			WorkingSetCache.impl.fetchTaxaForWorkingSet(recentWorkingSet, new GenericCallback<String>() {
				...
				public void onSuccess(String arg0) {
					...
					for (String taxaID : recentWorkingSet.getSpeciesIDs()) {
						...
						fullNames.put(taxaName, taxaID);
						pagingLoader.getFullList().add(new TaxaData(taxaName, TaxaData.FULLNAME, taxaID, null));
					}
			...
			public void handleEvent(BaseEvent be) {
				if (storeBinder.getSelection() != null && storeBinder.getSelection().size() > 0) {
					...
					if (data.getType().equalsIgnoreCase(TaxaData.FULLNAME)) {
						TaxonomyCache.impl.fetchNode(data.getID(), true, new GenericCallback<TaxonNode>() {
							public void onFailure(Throwable caught) {
						...
						TaxonomyCache.impl.fetchNodeWithKingdom(data.getID(), data.getName(), true,

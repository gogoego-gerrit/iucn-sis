...
import org.iucn.sis.client.components.panels.FileUploadWidget;
	...
	private void init(String id) {
		...
		item.addSelectionListener(new SelectionListener<ButtonEvent>() {
			public void componentSelected(ButtonEvent ce) {
				...
				addPopup.add(new FileUploadWidget("/images/" + groupingId) {
...
//				s.setLayout(new FitLayout());
	...
	public void update() {
		...
		ndoc.get("/images/" + groupingId, new GenericCallback<String>() {
			...
			public void onSuccess(String result) {
				...
				if (imageNodes.getLength() == 0)
					...
					for (int i = 0; i < imageNodes.getLength(); i++) {
						...
						image.setImage(new Image("/raw/images/bin/" + image.getId()
	...
	protected void writeImagesToFS() {
		...
		newDoc.put("/images/" + groupingId, xml, new GenericCallback<String>() {
			public void onFailure(Throwable caught) {
				
			}

			public void onSuccess(String result) {
				ClientUIContainer.bodyContainer.tabManager.panelManager.taxonomicSummaryPanel.update(groupingId);
				update();
			}
		});
	}

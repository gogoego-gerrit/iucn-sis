...
public class BatchChangePanel extends LayoutContainer {
	...
	private AssessmentData template;
	...
	private void build() {
		...
		//		types.addItem("", "");
		//		types.addItem("Global Draft Assessments", BaseAssessment.DRAFT_ASSESSMENT_STATUS);
		//		types.addItem("Current Published Assessments", BaseAssessment.PUBLISHED_ASSESSMENT_STATUS);
		...
		workingSets.addChangeListener(new ChangeListener() {
			public void onChange(Widget sender) {
				if (workingSets.getSelectedIndex() == 0) {
					...
					AssessmentFilter filter = WorkingSetCache.impl.getWorkingSet("" + workingSets.getValue(workingSets.getSelectedIndex())).getFilter();
	...
	private void onSubmit() {
		...
		if (fields.length == 0) {
			...
			for (int i = 0; i < fields.length; i++)
				if (template.getDataMap().containsKey(fields[i].getText()))
					newData.put(fields[i].getText(), template.getDataMap().get(fields[i].getText()));
				else
					newData.put(fields[i].getText(), null);

			final WorkingSetData ws = WorkingSetCache.impl.getWorkingSet(""
					+ workingSets.getValue(workingSets.getSelectedIndex()));
	...
	public void refresh() {
		...
		for (int i = 0; i < CanonicalNames.batchChangeSubset.length; i++) {
			...
			fieldStore.add(new FieldListElement(name, template.getDataMap().containsKey(
					CanonicalNames.batchChangeSubset[i])));
		}
	...
	private void startUpdates(final HashMap newData, final WorkingSetData ws) {

		final AssessmentFilter filter = filterPanel.getFilter();

		if (filter.getRegionType().equalsIgnoreCase(AssessmentFilter.REGION_TYPE_ALL) || filter.getRegionType().equalsIgnoreCase(AssessmentFilter.REGION_TYPE_OR))
		{
...
//				String draftIDs = null;
				...
				{
					taxaIDs = "";
					for (Iterator<String> iter = ws.getSpeciesIDs().listIterator(); iter.hasNext();) {
						TaxonNode curTaxa = TaxonomyCache.impl.getNode(iter.next().toString());
				...
				if (taxaIDs != null) {
					AssessmentData ass = template.deepCopy();
					ass.setType(template.getType());
					ass.setData(newData);
					ass.setReferences(new HashMap());
					...
					batchAway.post("/batchChange", xml.toString(), new GenericCallback<String>() {

...
public class RegionPanel extends ContentPanel {
	...
	public void build(boolean enableEditing) {
		...
		grid.getSelectionModel().addListener(Events.SelectionChange, new Listener<SelectionEvent<RegionModel>>() {
			public void handleEvent(SelectionEvent<RegionModel> be) {
				if (be.getModel() != null) {
					boolean globalEnabled = !be.getModel().getRegion().getId().equals(BaseAssessment.GLOBAL_ID);
					
					delete.setEnabled(globalEnabled);
					name.setEnabled(globalEnabled);
					description.setEnabled(globalEnabled);
					
					formBindings.bind(be.getModel());
					name.setAllowBlank(false);
					description.setAllowBlank(false);

					if (be.getModel().get("id").equals(Region.DEFAULT_NEW_ID))
...
// setAlignment(HorizontalAlignment.LEFT);
		add = new Button("Add New Region", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				if (isPanelDataValid()) {
					RegionModel newone = new RegionModel(new Region(Region.DEFAULT_NEW_ID, "", ""));
					store.add(newone);
		...
		store.addStoreListener(new StoreListener<RegionModel>() {
			...
			public void storeUpdate(StoreEvent<RegionModel> se) {
				...
				if (se.getOperation() == RecordUpdate.REJECT) {
					if (se.getModel().get("id").equals(Region.DEFAULT_NEW_ID)) {
	...
	private FormPanel createForm() {
		...
		name.setValidator(new Validator() {
			public String validate(Field<?> field, String value) {
				...
				else if (grid.getSelectionModel().getSelectedItem().get("id").equals(Region.DEFAULT_NEW_ID)

package org.iucn.sis.client.components.panels.filters;
...
//	protected boolean admin;
	...
	public void setFilter(AssessmentFilter filter) {
		...
		regionPanel.setRegionsSelected(filter.getRegions());
		regionPanel.refreshUI();
		if (allowMatchesAnyRegion && filter.getRegionType().equalsIgnoreCase(AssessmentFilter.REGION_TYPE_OR))
		...
		else
		{
			...
			{ 
				if (entry != null)
				{
					...
					filter.getRegions().add(entry.getValue().getRegion().getId());
				}	
			}
			
			if (box.getSimpleValue().equalsIgnoreCase(all)) {
				filter.setRegionType(AssessmentFilter.REGION_TYPE_AND);
				System.out.println("setting to and in filter " + filter );
			}
			else {
				filter.setRegionType(AssessmentFilter.REGION_TYPE_OR);

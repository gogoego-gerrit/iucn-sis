...
import org.iucn.sis.client.components.DEMToolsPopups;
import org.iucn.sis.client.components.panels.changes.AssessmentChangesPanel;
...
import org.iucn.sis.client.displays.SISPageHolder;
...
import org.iucn.sis.client.utilities.FilenameStriper;
import org.iucn.sis.client.workflow.WorkflowNotesWindow;
...
public class DEMPanel extends LayoutContainer {
	private class AutosaveTimer extends Timer {
		public void run() {
			...
			try {
				...
				boolean save = AssessmentUtilFactory.getSaveUtils().shouldSaveCurrentAssessment();
				if (save) {
	...
	private AssessmentData lastAssessmentShown = null;
	...
	private ToolBar buildToolBar() {
		...
		item.addSelectionListener(new SelectionListener<ButtonEvent>() {
			public void componentSelected(ButtonEvent ce) {
				...
				else if (TaxonomyCache.impl.getCurrentNode().getFootprint().length < TaxonNode.GENUS) {
		...
		item.addListener(Events.Select, new Listener() {
			public void handleEvent(BaseEvent be) {
				if (!AuthorizationCache.impl.hasRight(SimpleSISClient.currentUser, AuthorizableObject.WRITE, TaxonomyCache.impl.getCurrentNode())) {
					WindowUtils.errorAlert("Sorry. You do not have sufficient permissions " + "to perform this action.");
					return;
				}

				final AssessmentAttachmentPanel attachPanel = new AssessmentAttachmentPanel(AssessmentCache.impl.getCurrentAssessment().getAssessmentID());
			...
			public void componentSelected(MenuEvent ce) {
				...
				if (TaxonomyCache.impl.getCurrentNode().getCommonNames().size() != 0) {
					for (int i = 0; i < TaxonomyCache.impl.getCurrentNode().getCommonNames().size(); i++) {
						CommonNameData curName = (CommonNameData) TaxonomyCache.impl.getCurrentNode().getCommonNames()
						.get(i);
		...
		mItem.addListener(Events.Select, new Listener<BaseEvent>() {
			public void handleEvent(BaseEvent be) {
				AssessmentData a = AssessmentCache.impl.getCurrentAssessment();
				Window alert = WindowUtils.getWindow(false, false, "Assessment #" + a.getAssessmentID());
				...
				String target = "/comments/browse/assessment/"
					+ FilenameStriper.getIDAsStripedPath(a.getAssessmentID()) + ".comments.xml";
		...
		workflow.addSelectionListener(new SelectionListener<MenuEvent>() {
			public void componentSelected(MenuEvent ce) {
				final WorkflowNotesWindow window = 
					new WorkflowNotesWindow(AssessmentCache.impl.getCurrentAssessment().getAssessmentID());
				window.show();
			}
		});
	...
	private void doPageChangeRequested(final SISView curView, final SISPageHolder curPage, final HTML curPageLabel) {
		...
		if (!viewOnly && AssessmentUtilFactory.getSaveUtils().shouldSaveCurrentAssessment() && !samePage) {
			stopAutosaveTimer();
	...
	private void runIntegrityValidator() {
		final AssessmentData data = AssessmentCache.impl.getCurrentAssessment();
		//Popup new window:
		ClientAssessmentValidator.validate(data.getAssessmentID(), data.getType());
		/*ClientAssessmentValidator.validate(data.getAssessmentID(), data.getType(),
			...
			public void onSuccess(NativeDocument result) {
				final ValidationResultsWindow window = new ValidationResultsWindow(
						data.getAssessmentID(), result.getText());
				window.show();
			}
			public void onFailure(Throwable caught) {
			}
	...
	private void fetchReport() {
		...
		form.addButton(new Button("Submit", new SelectionListener<ButtonEvent>() {
			public void componentSelected(ButtonEvent ce) {
				...
				if (a.getType().equals(BaseAssessment.DRAFT_ASSESSMENT_STATUS)) {
					target += "draft/";
				} else if (a.getType().equals(BaseAssessment.PUBLISHED_ASSESSMENT_STATUS)) {
					target += "published/";
				} else if (a.getType().equals(BaseAssessment.USER_ASSESSMENT_STATUS)) {
					target += "user/" + SimpleSISClient.currentUser.username + "/";
				}

				w.close();
				
				com.google.gwt.user.client.Window.open(target + AssessmentCache.impl.getCurrentAssessment().getAssessmentID()
...
w.setSize(400, 250);
	...
	public void resetAutosaveTimer() {
		SysDebugger.getInstance().println("Resetting autosave.");
		autoSave.cancel();
		startAutosaveTimer();
	}

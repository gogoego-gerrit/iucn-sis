...
import org.iucn.sis.client.utilities.TaxonNodeComparator;
...
public class TaxonomyBrowserPanel extends LayoutContainer {
	...
	public static class TaxonListElement extends BaseModel {
		TaxonNode node;
		String footprint;
		...
		public TaxonListElement(TaxonNode node, String footprint) {
			this.node = node;
			this.footprint = footprint;
			set("name", node.getName() + (node.getStatus().matches("(^[Nn]$)|(^New$)") ? "*" : ""));
			set("fullName", node.getFullName());
			set(TaxonNodeComparator.SEQ_CODE, String.valueOf(node.getSequenceCode()));
		}
		...
		public void setSequenceCode(String code) {
			set(TaxonNodeComparator.SEQ_CODE, code);
		}
	...
	protected TaxonNode lastTaxon = null;
	...
	public TaxonomyBrowserPanel() {
		...
		final StoreSorter<TaxonListElement> sorter = new StoreSorter(TaxonNodeComparator.getInstance());
	...
	protected void addViewButtonToFootprint() {
		Button view = new Button("View", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				TaxonomyCache.impl.fetchNode(footprints[footprints.length - 1], true, new GenericCallback<TaxonNode>() {
	...
	protected HTML getClickableHTML(final String label, final String path) {
		...
		if (!label.equalsIgnoreCase("Kingdoms")) {
			TaxonomyCache.impl.fetchNode(label, false, new GenericCallback<TaxonNode>() {
				public void onFailure(Throwable caught) {
				}
	...
	protected void updateBody(NativeDocument ndoc, String footprint, int offset) {
		...
		if (options.getLength() == 0)
			optionsPanel.add(new HTML("No " + TaxonNode.getDisplayableLevel(footprint.split("-").length) + "."));
		else {
			if (footprint.equals("")) {
				optionsPanel.add(new HTML("<b>Kingdom</b><br />"));
			} else {
				optionsPanel.add(new HTML("<b>" + TaxonNode.getDisplayableLevel(footprint.split("-").length)
						+ "</b><br />"));
			}
			...
			if (asCheckable) {
				if (footprints.length >= TaxonNode.ORDER) {
			...
			if (csv.length() > 0) {
				TaxonomyCache.impl.fetchList(csv.substring(0, csv.length() - 1), new GenericCallback<String>() {
					public void onFailure(Throwable caught) {
					}

					public void onSuccess(String arg0) {
						for (int i = 0; i < options.getLength(); i++) {
							TaxonNode node = TaxonomyCache.impl.getNode(options.elementAt(i).getText());
							if( node != null ) {
		...
		if (footprints.length == 0)
			...
			for (int i = 0; i < footprints.length; i++) {
				...
				if (i < footprints.length - 1)
					...
					curHTML = new HTML(TaxonomyCache.impl.getNode(footprints[footprints.length - 1]).getName());
	...
	protected void updateNodeSummary() {
		...
		if (nodeID != "")
			ClientUIContainer.bodyContainer.tabManager.panelManager.taxonomicSummaryPanel.update(nodeID);
	}

...
import org.iucn.sis.client.acl.ui.PermissionGroupEditor;
import org.iucn.sis.client.components.panels.BatchChangePanel;
import org.iucn.sis.client.components.panels.DefinitionEditorPanel;
import org.iucn.sis.client.components.panels.FindReplacePanel;
import org.iucn.sis.client.components.panels.RedlistPanel;
import org.iucn.sis.client.components.panels.RegionPanel;
import org.iucn.sis.client.components.panels.TaxonFinderPanel;
import org.iucn.sis.client.components.panels.TrashBinPanel;
import org.iucn.sis.client.data.assessments.AssessmentCache;
import org.iucn.sis.client.integrity.IntegrityApplicationPanel;
import org.iucn.sis.client.locking.LockManagementPanel;
import org.iucn.sis.client.simple.SimpleSISClient;
import org.iucn.sis.client.simple.SimpleSISClient.SimpleSupport;
...
public class HeaderContainer extends LayoutContainer {
	...
	public HeaderContainer(String first, String last, String affiliation) {
		...
		rightData = new BorderLayoutData(LayoutRegion.EAST, 200f);
	...
	public void assessmentChanged() {
		AssessmentData curAssessment = AssessmentCache.impl.getCurrentAssessment();
		TaxonNode curNode = TaxonomyCache.impl.getCurrentNode();

		if (curAssessment == null)
			update();
		else if (curNode != null && curAssessment.getSpeciesID().equals(curNode.getId() + "")) {
			ClientUIContainer.bodyContainer.tabManager.panelManager.DEM.redraw();
			update();
		} else {
			ClientUIContainer.bodyContainer.tabManager.panelManager.DEM.redraw();
			TaxonomyCache.impl.fetchNode(curAssessment.getSpeciesID(), true, new GenericCallback<TaxonNode>() {
	...
	private LayoutContainer buildLeftPanel() {
		...
		if (SimpleSISClient.currentUser.getBusinessUnit().equalsIgnoreCase("birdlife"))
			leftImage.setUrl("images/logo-birdlifeStacked.gif");
		else
			leftImage.setUrl("images/sislogo_cropped.png");

		// leftImage.setSize("138px", "65px");
	...
	private LayoutContainer buildRightPanel(String first, String last, String affiliation) {
		...
		userIcon.addClickListener(new ClickListener() {
			public void onClick(Widget sender) {
				...
				profileDoc.get("/profile/" + SimpleSISClient.currentUser.username, new GenericCallback<String>() {
					public void onFailure(Throwable caught) {
					}
			...
			public void handleEvent(BaseEvent be) {
				...
				if( SimpleSISClient.iAmOnline ) {
					...
					if (AuthorizationCache.impl.hasRight(SimpleSISClient.currentUser, 
						...
						final String target = "/demimport/submit/" + SimpleSISClient.currentUser.username;
						...
						tabItem.getHeader().addListener(Events.OnClick, new Listener<BaseEvent>() {
							public void handleEvent(BaseEvent be) {
								...
								TaxonomyCache.impl.resetCurrentNode();

								TaxonomyCache.impl.clear();
								AssessmentCache.impl.clearDrafts();
...
//				UserModelTabPanel modelTabPanel = new UserModelTabPanel();
//				s.add(modelTabPanel);
//				
//				PermissionGroupEditor ed = new PermissionGroupEditor();
//				s.add(ed);
	...
	public void taxonChanged() {
		...
		if (curNode == null || (curAssessment != null && !curAssessment.getSpeciesID().equals(curNode.getId() + ""))) {
			AssessmentCache.impl.resetCurrentAssessment();
		}

		// Make sure working set isn't wrong.
		if (curNode != null) {
			if (curSet != null && !curSet.getSpeciesIDs().contains(curNode.getId() + ""))
	...
	public void workingSetChanged() {
		...
		if (curSet != null && curNode != null && !curSet.getSpeciesIDs().contains(curNode.getId() + ""))
			TaxonomyCache.impl.resetCurrentNode(); // Will also reset
		// curAssessment
		else
			update();

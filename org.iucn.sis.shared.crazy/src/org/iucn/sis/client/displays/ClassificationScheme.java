...
public class ClassificationScheme extends Display {
	public class ClassificationSchemeEntry extends BaseModel implements Referenceable {
		...
		public ArrayList<ReferenceUI> getReferencesAsList() {
			return AssessmentCache.impl.getCurrentAssessment().getReferences(entryCanonicalName);
		}
		...
		public void removeReferences(ArrayList<ReferenceUI> references, GenericCallback<Object> callback) {
			...
			for (int i = 0; i < references.size(); i++)
				if (AssessmentCache.impl.getCurrentAssessment().removeReference((ReferenceUI) references.get(i),
						entryCanonicalName))
					removed++;
	...
	private ArrayList curRefs = null;
	private ArrayList<Note> curNotes = null;
...
//		collapseAll.setIconStyle("");
	...
	protected Widget generateContent(boolean viewOnly) {
		...
		if (displayPanel == null) {
			...
			innerContainer.setAutoHeight(true);
			...
			pagingLoader.getPagingLoader().addLoadListener(new LoadListener() {
				...
				@Override
				public void loaderLoad(LoadEvent le) {
					System.out.println("Fired loaderLoad event.");
					if( grid.isRendered() ) {
						System.out.println("Fired loaderLoad event - isRendered.");
						if( grid.getHeight() <= 30 && grid.getStore().getCount() > 0 ) {
							System.out.println("new height " + gridH);
		...
		grid.getSelectionModel().addListener(Events.SelectionChange,  
				new Listener<SelectionChangedEvent<ClassificationSchemeEntry>>() {  
			public void handleEvent(SelectionChangedEvent<ClassificationSchemeEntry> be) {  
		...
		for (int i = 0; i < entries.length; i++) {
			...
			if (!codeToSelectedDesc.containsKey(curEntry.getKey())) {
				...
				if (canonicalName.equals(CanonicalNames.Threats) && curEntry.getKey().equals("0")) {
					ArrayList<String> temp = new ArrayList<String>();
					temp.add("true");

					AssessmentCache.impl.getCurrentAssessment().getDataMap().put(CanonicalNames.NoThreats, temp);
					FieldWidgetCache.impl.addAssessmentDataToDisplay(FieldWidgetCache.impl
							.get(CanonicalNames.NoThreats));
	...
	public ToolBar getLabelPanel(final Grid grid){
		...
		toolBar.addListener(Events.Change, new Listener<BaseEvent>(){
			public void handleEvent(BaseEvent be) {
				...
				curRefs = AssessmentCache.impl.getCurrentAssessment().getReferences(
						canonicalName + "." + ((ClassificationSchemeEntry)grid.getSelectionModel().getSelectedItem()).getKey());
		...
		refIcon.addSelectionListener(new SelectionListener<IconButtonEvent>() {
			public void componentSelected(IconButtonEvent ce) {
				Window s = WindowUtils.getWindow(false, true, "Add a references to " + canonicalName + " "
				...
				.setReferences(((ClassificationSchemeEntry)grid.getSelectionModel().getSelectedItem()));
				ClientUIContainer.bodyContainer.tabManager.panelManager.refViewPanel.setOnAddSelected(new GenericCallback<Object>() {
					...
					public void onSuccess(Object result) {
						...
						ReferenceCache.getInstance().addReferences(
								AssessmentCache.impl.getCurrentAssessment().getAssessmentID(), references);
				...
				ClientUIContainer.bodyContainer.tabManager.panelManager.refViewPanel.setOnRemoveSelected(
					...
					public void onSuccess(Object result) {
						...
						for (int i = 0; i < list.size(); i++) {
							AssessmentCache.impl.getCurrentAssessment().removeReference(
									(ReferenceUI) list.get(i), canonicalName + "." + ((ClassificationSchemeEntry)grid.getSelectionModel().getSelectedItem()).getKey());
						}
...
//		};
	 ...
	 */
	public void setSelectedAsStructures(HashMap newData) {
		selected.clear();
	...
	public void setSelectedFromData(HashMap<String, ArrayList<String>> newData) {
		...
		boolean occurrence = CanonicalNames.CountryOccurrence.equalsIgnoreCase(canonicalName)
		|| CanonicalNames.FAOOccurrence.equalsIgnoreCase(canonicalName)
		|| CanonicalNames.LargeMarineEcosystems.equalsIgnoreCase(canonicalName);
		...
		if (newData != null) {
			for (Entry<String, ArrayList<String>> curSelected : newData.entrySet()) {
				try {
					...
					if (occurrence) {
						ArrayList<String> data = curSelected.getValue();

						if (data.size() == 3) {
							OccurrenceMigratorUtils.modifyOccurrenceEntry(curSelected);
						}
					}


					curStruct.setData(newData.get(curKey), 0);

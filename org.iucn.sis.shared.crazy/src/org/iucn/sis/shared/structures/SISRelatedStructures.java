...
public class SISRelatedStructures extends DominantStructure {
	...
	private ArrayList dependantStructures;
	...
	public SISRelatedStructures(String structure, String description, DominantStructure dominantStructure,
	...
	public void addListenerToActiveStructure(ChangeListener changeListener, ClickListener clickListener,
		 ...
		 * new ClickListener() { public void onClick(Widget sender) {
		 ...
		 * ((Button)dominantStructure.getActiveStructure()).addClickListener(new
		 * ClickListener() { public void onClick(Widget sender) {
		 * updateDependantPanel(); } }); } else if
		 * (dominantStructure.structure.equalsIgnoreCase
	...
	public int setData(ArrayList dataList, int dataOffset) {
		for (int i = 0; i < dependantStructures.size() + 1; i++) {
			if (i == 0){
				dataOffset = dominantStructure.setData(dataList, i + dataOffset);
				model = new BaseModel(dominantStructure.extractModelData().getProperties());
			}
			else{
				dataOffset = ((Structure) dependantStructures.get(i - 1)).setData(dataList, dataOffset);

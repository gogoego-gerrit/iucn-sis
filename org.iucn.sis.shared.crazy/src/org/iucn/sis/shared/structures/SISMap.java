...
public class SISMap extends Structure {
	...
	public SISMap(String struct, String descript, Object data) {
	...
	private void createMapPoints() {
		 ...
		 * eventManager.addOnClickListener(current, new
		 * GMarkerEventClickListener() { public void onClick(GMarker marker) {
	...
	private Widget createNewPointInterface() {
		...
		createPoint.addClickListener(new ClickListener() {
	...
	private void createTextInputArea(String title, boolean canSave) {
		...
		for (i = 0; i < myMapData.size(); i++) {
			...
			removeMe.addClickListener(new ClickListener() {

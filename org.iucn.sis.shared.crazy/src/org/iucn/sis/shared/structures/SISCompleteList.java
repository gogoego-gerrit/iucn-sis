...
import org.iucn.sis.client.components.panels.workingsets.WorkingSetNewWSPanel;
...
public class SISCompleteList extends VerticalPanel {
	...
	private final WorkingSetNewWSPanel workingSetPanel;
	...
	public void addItemInList(String text) {
		...
		if (!listText.contains(text) && !text.trim().equalsIgnoreCase("")) {
			...
			if (workingSetPanel != null) {
				workingSetPanel.resize();
				// ((WorkingSetNewWSPanel)
				// getParent().getParent().getParent().getParent()).resize();
			}
		}
	}

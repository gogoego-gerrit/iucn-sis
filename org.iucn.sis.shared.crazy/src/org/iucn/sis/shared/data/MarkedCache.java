...
public class MarkedCache {
	...
	private void parseXML(NativeDocument doc) throws NullPointerException {
		...
		for (int i = 0; i < list.getLength(); i++) {
			...
			String[] info = list.item(i).getTextContent().split(",");
			markedAssessments.put(info[0] + "!" + BaseAssessment.DRAFT_ASSESSMENT_STATUS, info[1]);
		}

		list = published.getElementsByTagName("assessment");
		for (int i = 0; i < list.getLength(); i++) {
			String[] info = list.item(i).getTextContent().split(",");
			markedAssessments.put(info[0] + "!" + BaseAssessment.PUBLISHED_ASSESSMENT_STATUS, info[1]);
		}

		list = user.getElementsByTagName("assessment");
		for (int i = 0; i < list.getLength(); i++) {
			String[] info = list.item(i).getTextContent().split(",");
			markedAssessments.put(info[0] + "!" + BaseAssessment.USER_ASSESSMENT_STATUS, info[1]);
	...
	private void save() {
		...
		ndoc.put("/mark/" + SimpleSISClient.currentUser.username, toXML(), new GenericCallback<String>() {
	...
	private String toXML() {
		...
		while (iter.hasNext()) {
			...
			if (allid.endsWith(BaseAssessment.DRAFT_ASSESSMENT_STATUS)) {
				drafts += "<assessment>" + id + "," + markedAssessments.get(allid) + "</assessment>\r\n";
			} else if (allid.endsWith(BaseAssessment.PUBLISHED_ASSESSMENT_STATUS)) {
				published += "<assessment>" + id + "," + markedAssessments.get(allid) + "</assessment>\r\n";
			} else if (allid.endsWith(BaseAssessment.USER_ASSESSMENT_STATUS)) {
	...
	public void update() {
		final NativeDocument ndoc = SimpleSISClient.getHttpBasicNativeDocument();
		ndoc.get("/mark/" + SimpleSISClient.currentUser.username, new GenericCallback<String>() {

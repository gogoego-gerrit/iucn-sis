 ...
 */
public class CString extends BaseColumn {
	...
	public Date getDate() throws ConversionException {
		if (s == null)
			return null; // a null String == a null Date
		java.util.Date d;
		try {
			d = new SimpleDateFormat(SQLDateHelper.sqlDateTimeFormat).parse(s);
		} catch (final ParseException p1) {
			try {
				d = new SimpleDateFormat(SQLDateHelper.sqlDateFormat).parse(s);
			} catch (final ParseException p2) {
				throw new ConversionException(
						"CString column does not contain a date in SQL format");
			}
		}
		return d;
	}

 ...
 */
@ThreadSafe
public class HSQLDBSession extends DBSession {

	private final DataSource ds;

	public HSQLDBSession(final String name, final DataSource ds) {
		this.ds = ds;
	}
	...
	protected Row _listColumns(Connection conn, String tableName, ExecutionContext ec, boolean captureData) throws UnsupportedOperationException {
		System.out.println("## HSQL Attempting list cols");
		try {
	...
	private Row rsToRow(final ResultSet rs) throws SQLException {
		...
		while (rs.next()) {
			...
			try {
				...
				System.err.println("Row " + name + " could not be converted!");
				e.printStackTrace();
			}
		}
		return row;

...
/**
 * InstallationRestlet.java
 * 
 * @author carl.scott
 * 
 */
public class InstallationRestlet extends Restlet {

	public InstallationRestlet(Context context) {
		super(context);
	...
	public void handle(Request request, Response response) {
		try {
			installApplication(request, response);
	...
	private void installApplication(Request request, Response response) throws ResourceException {
			...
			if (node.getNodeName().equals("class")) {
				...
				for (Node child : children) {
					if (child.getNodeName().equals("property"))
						properties.put(DocumentUtils.impl.getAttribute(node, "name"), child.getTextContent());
				}
				...
				if (name.equals("")) {
					try {
						...
						throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST, "Classname invalid", e);
					}
				}

				try {
					GoGoDebug.get("debug").println("installing {0} as {1}", className, name);
					manager.install(className);
	...
	public static class InstallationClass implements Runnable {

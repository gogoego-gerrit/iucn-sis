	...
	public Representation represent(Variant variant) throws ResourceException {
		...
		if (form.getNames().contains("id")) {
			...
			for (int i = 0; i < id.length; i++) {
				try {
					info.add(new AssessmentInfo(id[i], type[i]));
				} catch (IndexOutOfBoundsException e) {
					info.add(new AssessmentInfo(id[i], BaseAssessment.PUBLISHED_ASSESSMENT_STATUS));
				}
			}
		}
			...
			for (int i = 0; i < set.length; i++) {
				final WorkingSetData data = 
					WorkingSetIO.readPublicWorkingSetAsWorkingSetData(vfs, set[i]);
				
				AssessmentFilterHelper helper = new AssessmentFilterHelper(data.getFilter());
				for (String id : data.getSpeciesIDs()) {
//					info.add(new AssessmentInfo(id, BaseAssessment.DRAFT_ASSESSMENT_STATUS)); OLD WAY
					for( AssessmentData curAss : helper.getAssessments(id, vfs) )
						if( curAss.isDraft() ) 
							info.add(new AssessmentInfo(curAss.getAssessmentID(), BaseAssessment.DRAFT_ASSESSMENT_STATUS));
				}
			}
		} else
		...
		final Collection<AssessmentInfo> assessmentInfo = new HashSet<AssessmentInfo>(); {
			...
			for (Node node : nodes)
				if ("assessment".equals(node.getNodeName()))
					assessmentInfo.add(new AssessmentInfo(node.getTextContent(),BaseDocumentUtils.impl.getAttribute(node, "type")));
		}
	...
	protected boolean runAssessment(final String rulesetName, final Document ruleset, final Document response, final String speciesName, final String assessmentID, 
	...
	private boolean isAssessmentInDatabase(String assessmentID, String assessmentType) {
	...
	protected String getSpeciesName(AssessmentInfo info) {
		AssessmentData data = AssessmentIO.readAssessment(vfs, info.getID(), info.getType(), null);
		if (data == null) {
	...
	protected static class AssessmentInfo {
		...
		public AssessmentInfo(String id, String type) {
			this.id = id;
			this.type = type;
			this.name = null;
		}

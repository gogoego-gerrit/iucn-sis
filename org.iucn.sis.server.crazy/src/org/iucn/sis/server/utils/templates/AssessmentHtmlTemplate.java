...
import org.iucn.sis.shared.view.View;
...
public class AssessmentHtmlTemplate {
	...
	private void buildAssessmentInfo(AssessmentData assessment, VFS vfs) {
	...
	// Conservation
	private void buildBibliography(AssessmentData assessment, VFS vfs) {
		...
		if( assessment.getReferences().size() == 0 ) {
			...
			ArrayList<ReferenceUI> seen = new ArrayList<ReferenceUI>();
			ArrayList<ReferenceUI> allRefs = assessment.getReferencesAsList();
			final AlphanumericComparator comparer = new AlphanumericComparator();
			
			ArrayUtils.quicksort(allRefs, new Comparator<ReferenceUI>() {
				public int compare(ReferenceUI o1, ReferenceUI o2) {
			...
			for( ReferenceUI curRef : allRefs ) {
	...
	private void buildCommonNames(TaxonNode taxon) {
	...
	// Conservation
	private void buildConservation(Page conservationPage, AssessmentData assessment, VFS vfs) {
	...
	// Habitats
	private void buildHabitats(Page habitatsPage, AssessmentData assessment, VFS vfs) {
	...
	/**** Header ****/
	private void buildHeadingTable(AssessmentData assessment) {
		theHtml.append("<title>");
	...
	private void buildHierarchyList(TaxonNode taxon) {
	...
	// Occurrence
	private void buildOccurrence(Page occurrencePage, AssessmentData assessment, VFS vfs) {
	...
	// Habitats
	private void buildPage(Page page, String pageName, AssessmentData assessment, VFS vfs) {
	...
	private StringBuffer buildPageOrganizations(ArrayList<Organization> organizations, AssessmentData assessment,
			VFS vfs) {
	...
	// Population
	private void buildPopulation(Page populationPage, AssessmentData assessment, VFS vfs) {
	...
	private void buildRedListAssessment(Page redListPage, AssessmentData assessment, VFS vfs) {
	...
	// Services
	private void buildServices(Page servicesPage, AssessmentData assessment, VFS vfs) {
	...
	private void buildSynonyms(TaxonNode taxon) {
	...
	private void buildTaxonomyInformation(AssessmentData assessment) {
	...
	// Threats
	private void buildThreats(Page threatsPage, AssessmentData assessment, VFS vfs) {
	...
	// Use and Trade
	private void buildUseTrade(Page useTradePage, AssessmentData assessment, VFS vfs) {
...
//		System.out.println("add label formatting to '" + label + "'");
	...
	protected DisplayData fetchDisplayData(String canonicalName, VFS vfs) {
		try {
			...
			jnd.parse(DocumentUtils.serializeNodeToString(DocumentUtils.getVFSFileAsDocument("/browse/docs/fields/"
	...
	public void parse(AssessmentData assessment, TaxonNode node, View curView) {
		...
		if (node == null) {
			...
			taxon = TaxonNodeFactory.createNode(taxaDoc);
		} else
			taxon = node;

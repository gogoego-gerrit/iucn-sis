...
public class AssessmentTemplate implements ReportTemplate {
	...
	private void buildHierarchyTable(TaxonNode taxon) {
		...
		for (int i = 0; i < taxon.getFootprint().length; i++) {
			hierarchyInnerTable.addCell(new Phrase(TaxonNode.getDisplayableLevel(i), timesSmall));
			hierarchyInnerTable.addCell(new Phrase(taxon.getFootprint()[i], timesSmall));
		}

		hierarchyInnerTable.addCell(new Phrase(TaxonNode.getDisplayableLevel(taxon.getLevel()), timesSmall));
	...
	protected DisplayData fetchDisplayData(String canonicalName, VFS vfs) {
		NativeDocument jnd = SISContainerApp.newNativeDocument(null);
	...
	public void parse(AssessmentData assessment, TaxonNode node) {
		document.open();
		...
		try {
			...
			if (!(readValue > 0))
				SysDebugger.getInstance().println("No Image Read.");
		...
		try {
			if (node == null) {
				...
				NativeDocument taxaDoc = SISContainerApp.newNativeDocument(null);
				taxaDoc.parse(sInfo);

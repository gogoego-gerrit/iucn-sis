...
import org.iucn.sis.server.utils.WordUtils;
	...
	private void buildAssessments() throws Exception {
		...
		for (Iterator<Long> iter = assessedNodesBySpc_id.keySet().iterator(); iter.hasNext();) {
			...
			if( curAssessment.isGlobal() ) {
				if( AssessmentIO.readAssessment(SISContainerApp.getStaticVFS(), curAssessment.getAssessmentID(), 
						BaseAssessment.DRAFT_ASSESSMENT_STATUS, null) != null ) {
				...
				List<AssessmentData> assessments = AssessmentIO.readAllDraftAssessments(
						SISContainerApp.getStaticVFS(), curNode.getId()+"");
				
				for( AssessmentData curExtant : assessments ) {
					if( curExtant.getRegionIDs().equals(curAssessment.getRegionIDs() ) ) {
			...
			for (HashMap curLevel : curKingdom.getLevels()) {
				for (Iterator<TaxonNode> iter = curLevel.values().iterator(); iter.hasNext();) {
					...
					if (cur.getId() == newNodeID) // SUBMIT IT
					{
						...
						if( TaxaIO.writeNode(cur, SISContainerApp.getStaticVFS()) == null ) {
							failed.set(true);
...
//			Generation length - DO NOT USE GEN_LENGTH FROM THIS TABLE!!!!!!!!!!!!!!!!!!!!!!
	...
	private void systematicsTableImport(Long curDEMid, AssessmentData curAssessment, HashMap<String, Object> data)
		...
		if (!regionName.equalsIgnoreCase("Global")) {
			...
			if (!response.getStatus().isSuccess())
				SysDebugger.getInstance().println("Error getting Region for " + regionName);

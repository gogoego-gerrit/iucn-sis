...
public class GAADEMImport implements Runnable {
	...
	private ArrayList<TaxonNode> changedNodes;
	...
	public GAADEMImport(File source, Uniform uniform, String fileName, String user) {
		this.source = source;
	...
	private void addTaxaDetails() throws DBException {
		for (Long curSpcID : assessedNodesBySpc_id.keySet()) {
			TaxonNode curNode = assessedNodesBySpc_id.get(curSpcID);

			curNode.setCommonNames(new ArrayList());
			curNode.setSynonyms(new ArrayList());
			...
			for (Iterator<Row> rows = rowSet.getSet().iterator(); rows.hasNext();) {
				...
				commonName.setValidated(true, CommonNameData.UNSET);

				// boolean alreadyExists = false;
				//				
				// for(Iterator cnIter =
				// curNode.getCommonNames().listIterator();
				// cnIter.hasNext() && !alreadyExists; )
				// {
			...
			for (Iterator<Row> rows = rowSet.getSet().iterator(); rows.hasNext();) {
				...
				String status = curRow.get("Synonym_status").getString(Column.NEVER_NULL);
				String notes = curRow.get("SynonymNotes").getString(Column.NEVER_NULL);

				SynonymData curSyn = new SynonymData("", name, "", TaxonNode.INFRARANK_TYPE_NA, TaxonNode.SPECIES,
						null, status, notes, "");
				// SynonymData curSyn = new SynonymData(name, TaxonNode.SPECIES,
				// null, status, notes, "");

				String authority = curRow.get("Syn_Authority").getString(Column.NEVER_NULL);
				int level;
				if (authority.contains("ssp.") || authority.contains("var."))
					level = TaxonNode.INFRARANK;
				else
					level = TaxonNode.SPECIES;

				curSyn.setAuthority(authority, level);
				curSyn.setStatus("A");

				// boolean alreadyExists = false;
				//				
				// for(Iterator synIter = curNode.getSynonyms().listIterator();
				// synIter.hasNext() && !alreadyExists; )
				// {
	...
	private void buildAssessments() throws Exception {
		...
		System.out.println("Going to build " + assessedNodesBySpc_id.size() + " assessments.");
		...
		for (Iterator<Long> iter = assessedNodesBySpc_id.keySet().iterator(); iter.hasNext();) {
			...
			curAssessment.setType(BaseAssessment.PUBLISHED_ASSESSMENT_STATUS);

			HashMap<String, Object> data = new HashMap<String, Object>();

			// Set consistency check to Done
			data.put(CanonicalNames.RedListConsistencyCheck, createDataArray(new String[] { "2",
					FormattedDate.impl.getDate(), "1", "", "" }));
	...
	private TaxonNode buildClass(ArrayList<String> footprint, TaxonNode phylumN, String curKingdom, String curPhylum,
			String curClass, String curStat) throws Exception {
		TaxonNode classN = null;
		TaxonNode extantN = null;

		extantN = tree.getNode(TaxonNode.CLASS, curKingdom, curClass);
		if (extantN == null) {
			extantN = fetchNode(curKingdom, curClass);

			if (classN == null) {
				classN = TaxonNodeFactory.createNode((long) newNodeID, curClass, TaxonNode.CLASS, "", curPhylum, false,
						curStat, "", "");
		...
		classN = whereDoIBelong(footprint, classN, extantN, phylumN);
	...
	private TaxonNode buildFamily(ArrayList<String> footprint, TaxonNode orderN, String curKingdom, String curOrder,
			String curFamily, String curStat) throws Exception {
		TaxonNode familyN = null;
		TaxonNode extantN = null;

		extantN = tree.getNode(TaxonNode.FAMILY, curKingdom, curFamily);
		if (extantN == null) {
			extantN = fetchNode(curKingdom, curFamily);
		...
		familyN = whereDoIBelong(footprint, familyN, extantN, orderN);
	...
	private TaxonNode buildGenus(ArrayList<String> footprint, TaxonNode familyN, String curKingdom, String curFamily,
			String curGenus, String curStat) throws Exception {
		TaxonNode genusN = null;
		TaxonNode extantN = null;

		extantN = tree.getNode(TaxonNode.GENUS, curKingdom, curGenus);
		if (extantN == null) {
			extantN = fetchNode(curKingdom, curGenus);

			if (genusN == null) {
				genusN = TaxonNodeFactory.createNode((long) newNodeID, curGenus, TaxonNode.GENUS, "", curFamily, false,
						curStat, "", "");
		...
		genusN = whereDoIBelong(footprint, genusN, extantN, familyN);
	...
	private TaxonNode buildKingdom(ArrayList<String> footprint, String curKingdom, String curStat) throws Exception {
		...
		if (kingdomN == null) {
			...
			if (kingdomN == null) {
				kingdomN = TaxonNodeFactory.createNode((long) newNodeID, curKingdom, TaxonNode.KINGDOM, "", "", false,
						curStat, "", "");
				kingdomN.setFullName(kingdomN.getName());

				kingdomN.setFootprint(new String[0]);

				String curXML = TaxonNodeFactory.nodeToDetailedXML(kingdomN);

				Request request = new Request(Method.PUT, uriPrefix + "/taxomatic/new", new StringRepresentation(curXML,
				...
				try {
					...
					kingdomN.setId(newID);
				} catch (Exception e) {
					failed.set(true);
					System.out.println("FAILED TO SAVE TAXON TO SYSTEM " + curXML);
					System.exit(0);
				}
			}
	...
	private TaxonNode buildOrder(ArrayList<String> footprint, TaxonNode classN, String curKingdom, String curClass,
			String curOrder, String curStat) throws Exception {
		TaxonNode orderN = null;
		TaxonNode extantN = null;

		extantN = tree.getNode(TaxonNode.ORDER, curKingdom, curOrder);
		if (extantN == null) {
			extantN = fetchNode(curKingdom, curOrder);

			if (orderN == null) {
				orderN = TaxonNodeFactory.createNode((long) newNodeID, curOrder, TaxonNode.ORDER, "", curClass, false,
						curStat, "", "");
		...
		orderN = whereDoIBelong(footprint, orderN, extantN, classN);
	...
	private TaxonNode buildPhylum(ArrayList<String> footprint, TaxonNode kingdomN, String curKingdom, String curPhylum,
			String curStat) throws Exception {
		TaxonNode phylumN = null;
		TaxonNode extantN = null;

		extantN = tree.getNode(TaxonNode.PHYLUM, curKingdom, curPhylum);
		if (extantN == null) {
			extantN = fetchNode(curKingdom, curPhylum);

			phylumN = TaxonNodeFactory.createNode((long) newNodeID, curPhylum, TaxonNode.PHYLUM, "", curKingdom, false,
					curStat, "", "");
		...
		phylumN = whereDoIBelong(footprint, phylumN, extantN, kingdomN);
	...
	private TaxonNode buildRank(TaxonNode extantN, ArrayList<String> footprint, TaxonNode speciesN, TaxonNode rankN,
			String curKingdom, String curGenus, String curSpecies, String curRank, String curSSP, String curStat,
			String infraTaxonomicAuthority, long specID, boolean hybrid) throws Exception {
		TaxonNode extantRank = null;

		if (curRank != null) {
			System.out.println("CRAP I RAN INTO AN INFRARANk!!!!!");
			System.exit(0);

			rankN = tree.getNode(TaxonNode.INFRARANK, curKingdom, curGenus + " " + curSpecies + " " + curRank);
			if (rankN == null) {
				if (extantN != null) {
					if (extantN.getLevel() == TaxonNode.INFRARANK)
						extantRank = extantN;
					else
						extantRank = fetchNode(curKingdom, extantN.getFootprint()[TaxonNode.GENUS]
								+ extantN.getFootprint()[TaxonNode.SPECIES]
								+ extantN.getFootprint()[TaxonNode.INFRARANK]);
				}
				extantRank = fetchNode(curKingdom, curGenus + curSpecies + curRank);

				if (rankN == null) {
					rankN = TaxonNodeFactory.createNode((long) newNodeID, curRank, TaxonNode.INFRARANK, "", curSpecies,
							false, curStat, "", "");

					rankN.setFullName(curGenus + " " + curSpecies + " " + curRank);
			...
			if (curSSP == null) {
				rankN = whereDoIBelong(footprint, rankN, extantN, null);
				assessedNodesBySpc_id.put(new Long(specID), rankN);
				rankN.setHybrid(hybrid);
			} else
				rankN = whereDoIBelong(footprint, rankN, extantRank, null);
	...
	private TaxonNode buildSpecies(TaxonNode extantN, ArrayList<String> footprint, TaxonNode genusN, String curKingdom,
			String curGenus, String curSpecies, String curRank, String curSSP, String curStat,
			String spcTaxonomicAuthority, long specID, boolean hybrid) throws Exception {
		TaxonNode speciesN = null;
		TaxonNode extantSpecies = null;

		extantSpecies = tree.getNode(TaxonNode.SPECIES, curKingdom, curGenus + " " + curSpecies);
		if (extantSpecies == null) {
			if (extantN != null) {
				if (extantN.getLevel() == TaxonNode.SPECIES)
					extantSpecies = extantN;
				else {
					System.out.println(" ******* WEIRD - Extant Node is of level " + extantN.getLevelString());
					System.exit(0);

					extantSpecies = fetchNode(curKingdom, extantN.getFootprint()[TaxonNode.GENUS]
							+ extantN.getFootprint()[TaxonNode.SPECIES]);
				}
			} else
				extantSpecies = fetchNode(curKingdom, curGenus + curSpecies);

			if (speciesN == null) {
				speciesN = TaxonNodeFactory.createNode((long) newNodeID, curSpecies, TaxonNode.SPECIES, "", curGenus,
						false, curStat, "", "");
		...
		if (curRank == null && curSSP == null) {
			speciesN = whereDoIBelong(footprint, speciesN, extantSpecies, genusN);
	...
	private TaxonNode buildSubpop(TaxonNode extantN, ArrayList<String> footprint, TaxonNode sspN, TaxonNode speciesN,
			TaxonNode rankN, String curKingdom, String curGenus, String curSpecies, String curRank, String curSSP,
			String curStat, String spcTaxonomicAuthority, long specID, boolean hybrid) throws Exception {
		if (curSSP != null) {
			System.out.println("CRAP I RAN INTO A SUBPOP!!!!!");
			System.exit(0);

			if (curRank == null)
				sspN = tree.getNode(TaxonNode.SUBPOPULATION, curKingdom, curGenus + " " + curSpecies + " " + curSSP);
			else
			...
			if (sspN == null) {
				...
				if (sspN == null) {
					...
					sspN.setFullName(curGenus + " " + curSpecies + " " + curSSP);
					spcNameToIDMap.put(sspN.getFullName(), new Long(specID));

					if (curRank != null)
						whereDoIBelong(footprint, rankN, extantN, null);
			...
			sspN = whereDoIBelong(footprint, sspN, extantN, null);
	...
	public void buildTree() throws Exception {
		...
		for (Iterator<Row> iter = rowList.listIterator(); iter.hasNext();) {
			...
			String SIS_code = curCol.get("SIS_code").getString(Column.NEVER_NULL);
			TaxonNode extantN = null;
			if (!SIS_code.equals("") && !SIS_code.equals("0") && SIS_code.matches("\\d*")) {
				try {
					extantN = fetchNode(SIS_code);
				} catch (Exception e) {
					e.printStackTrace();
					System.out.println("Error fetching node with ID " + SIS_code);
				}
			}
			...
			TaxonNode subpopN = null;

			String curKingdom = curCol.get("Kingdom").getString().trim();
			String curPhylum = curCol.get("Phylum").getString().trim();
			String curClass = curCol.get("Class").getString().trim();
			String curOrder = curCol.get("Order").getString().trim();
			String curFamily = curCol.get("Family").getString().trim();
			String curGenus = curCol.get("Genus").getString().trim();
			String curSpecies = curCol.get("Species").getString().trim();
			String curRank = null;
			String curSubpop = null;
			String curStat = "New";
			...
			if (curCol.get("Rank").getString(Column.NATURAL_NULL) != null
					&& curCol.get("Rank_epithet").getString(Column.NATURAL_NULL) == null) {
				failed.set(true);
			...
			if (curCol.get("Rank").getString(Column.NATURAL_NULL) != null) {
				curRank = curCol.get("Rank").getString(Column.NEVER_NULL);
				curRank += " " + curCol.get("Rank_epithet").getString(Column.NEVER_NULL);

				curRank = curRank.trim();
			}

			if (curCol.get("Rank_author").getString(Column.NATURAL_NULL) != null)
				spcTaxonomicAuthority = curCol.get("Rank_author").getString(Column.NATURAL_NULL);

			curSubpop = curCol.get("Sub_pop").getString(Column.NATURAL_NULL);
			if (curSubpop != null)
				curSubpop = curSubpop.trim();

			kingdomN = buildKingdom(footprint, curKingdom, curStat);
			updateFootprint(footprint, kingdomN);

			phylumN = buildPhylum(footprint, kingdomN, curKingdom, curPhylum, curStat);
			updateFootprint(footprint, phylumN);

			classN = buildClass(footprint, phylumN, curKingdom, curPhylum, curClass, curStat);
			updateFootprint(footprint, classN);

			orderN = buildOrder(footprint, classN, curKingdom, curClass, curOrder, curStat);
			updateFootprint(footprint, orderN);

			familyN = buildFamily(footprint, orderN, curKingdom, curOrder, curFamily, curStat);
			updateFootprint(footprint, familyN);

			genusN = buildGenus(footprint, familyN, curKingdom, curFamily, curGenus, curStat);
			updateFootprint(footprint, genusN);

			speciesN = buildSpecies(extantN, footprint, genusN, curKingdom, curGenus, curSpecies, curRank, curSubpop,
					curStat, spcTaxonomicAuthority, specID, hybrid);

			rankN = buildRank(extantN, footprint, speciesN, rankN, curKingdom, curGenus, curSpecies, curRank,
					curSubpop, curStat, infraTaxonomicAuthority, specID, hybrid);
			subpopN = buildSubpop(extantN, footprint, subpopN, speciesN, rankN, curKingdom, curGenus, curSpecies,
					curRank, curSubpop, curStat, spcTaxonomicAuthority, specID, hybrid);
		}
	}
	...
	private boolean doGAALifeHistory(Long curDEMid, HashMap<String, Object> data) {
		// gaaLifeHistory
		try {
			switchToDBSession("gaaLifeHistory");
	...
	private void exportAssessments() throws Exception {
		...
		System.out.println("Going to export " + assessments.size() + " assessments.");
		...
		for (Iterator<AssessmentData> iter = assessments.values().iterator(); iter.hasNext();) {
			...
			Response response = uniform.put(uriPrefix + "/assessments, new StringRepresentation(xml, MediaType.TEXT_XML, null,
					CharacterSet.UTF_8));
		...
		// Response response = uniform.put(uriPrefix+"/workingSet/public/" +
		// user,
		// new StringRepresentation(workingSetXML, MediaType.TEXT_XML));
		//
		// if (!response.getStatus().isSuccess())
		...
		for (Object curEntry : tree.getKingdoms().values()) {
			...
			for (HashMap curLevel : curKingdom.getLevels()) {
				for (Iterator<TaxonNode> iter = curLevel.values().iterator(); iter.hasNext();) {
					...
					{
						System.out.println(" ==== Shit. I found a node with a -1 ID. ==== ");
						System.exit(0);
						logNode(cur);
	...
	private TaxonNode fetchNode(String id) throws Exception {
		System.out.println("Fetching " + id);
		Response response = uniform.get(uriPrefix + "/browse/nodes/" + id);
	...
	private TaxonNode fetchNode(String kingdomName, String fullNameNoSpaces) throws Exception {
		// System.out.println("Looking for: " + uriPrefix+"/browse/taxonName/" +
		// kingdomName + "/"
		// + URLEncoder.encode(fullNameNoSpaces.replaceAll("\\s", ""),
		// "UTF-8"));
	...
	private ArrayList parseOutLifeHistoryData(String value) {
		if (value == null)
	...
	private void populationTableImport(Long curDEMid, AssessmentData curAssessment, HashMap<String, Object> data)
		...
		if (row != null) {
			...
			// IF EITHER ARE "", WE DON'T NEED A COMMA - WE'LL TREAT IT AS A
			// BEST
			// GUESS
	...
	private void redListingTableImport(Long curDEMid, AssessmentData curAssessment, HashMap<String, Object> data)
		...
		try {
			switchToDBSession("gaaRLTable");
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("ERROR switching to new GAA RedListTable.");
		}
		...
		for (Iterator<Row> rows = rowSet.getSet().iterator(); rows.hasNext();) {
			...
			// Set the evaluated tick box
			if (!curAssessment.getEvaluators().equals(""))
				data.put(CanonicalNames.RedListEvaluated, createDataArray(new String[] { "true",
						FormattedDate.impl.getDate(), "1", "", "" }));
	...
	private void referencesImport(Long curDEMid, AssessmentData curAssessment, HashMap<String, Object> data) {
		...
		try {
			...
			for (Element refEl : new ElementCollection(el.getElementsByTagName("reference"))) {
				...
				// result =
				// result.replaceAll("<\\?xml\\s*(version=.*)?\\s*(encoding=.*)?\\?>",
				// "");
		...
		try {
			...
			try {
				...
				registerDatasource("demSource", "jdbc:access:////usr/data/demSource.mdb",
						"com.hxtt.sql.access.AccessDriver", "", "");
				registerDatasource("gaaLifeHistory", "jdbc:access:////usr/data/GAA/lifeHistory.mdb",
						"com.hxtt.sql.access.AccessDriver", "", "");
				registerDatasource("gaaRLTable", "jdbc:access:////usr/data/GAA/GAArlTable.mdb",
						"com.hxtt.sql.access.AccessDriver", "", "");

				switchToDBSession("dem");

				log.append("<table border=\"1\"><tr>" + "<th>Level</th>" + "<th>Friendly Name</th>"
						+ "<th>Kingdom</th>" + "<th>Phylum</th>" + "<th>Class</th>" + "<th>Order</th>"
						+ "<th>Family</th>" + "<th>Genus</th>" + "<th>Species</th>" + "<th>Infrarank</th></tr>");

				if (!failed.get()) {
					try {
						changedNodes = new ArrayList<TaxonNode>();
						buildTree();
						System.out.println("Changed " + taxonomyChanges + " existing taxa.");
					} catch (Exception e) {
			...
			DEMImportInformation.addToQueue(new DEMImportInformation(new Date(), !failed.get(), statusMessage
					.toString(), fileName, user, log.toString()));
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("ERROR running GAA DEM Import.");
			TaxonomyDocUtils.toggleWriteBack(true);
		}
	}
	...
	private void saveTaxon(TaxonNode node) {
		String curXML = TaxonNodeFactory.nodeToDetailedXML(node);

		Response response = uniform.put(uriPrefix + "/browse/nodes/" + node.getId(), new StringRepresentation(curXML,
				MediaType.TEXT_XML, null, CharacterSet.UTF_8));

		if (!response.getStatus().isSuccess()) {
			System.out.println("FAILED TO SAVE TAXON TO SYSTEM " + curXML);
		} else
			System.out.println("Save synonym and common name changes to: " + node.getId());
	}
	...
	private void systematicsTableImport(Long curDEMid, AssessmentData curAssessment, HashMap<String, Object> data)
		...
		Boolean regional = new Boolean(row.get("Assessments").getString(Column.NEVER_NULL).equalsIgnoreCase("regional"));

		if (regional.booleanValue()) {
			Boolean endemic = new Boolean(row.get("Endemic_region").getInteger(Column.NEVER_NULL) == 1);

			int region = row.get("Region").getInteger(Column.NEVER_NULL);

			SelectQuery selectRegion = new SelectQuery();
			select.select("region_lookup_table", "*");
			select.constrain(new CanonicalColumnName("region_lookup_table", "Region_number"), QConstraint.CT_EQUALS,
					region);

			Row.Loader regionLoader = new Row.Loader();
			ec.doQuery(selectRegion, regionLoader);

			String regionName = "ErrorRetrievingRegion";
			try {
				regionName = regionLoader.getRow().get("Region_name").getString(Column.NEVER_NULL);
			} catch (Exception ignored) {
			}

			data.put(CanonicalNames.RegionInformation, createDataArray(regional.toString(), regionName, endemic
					.toString()));
		}
	...
	private TaxonNode whereDoIBelong(ArrayList<String> footprint, TaxonNode newlyBuiltTaxon, TaxonNode extantNode,
			TaxonNode parentNode) {
		if (newlyBuiltTaxon == null) // We're all good.
		{
			// parentNode.addChild(extantNode);
			return extantNode;
		}

		if (extantNode != null) // If I exist...
		{
			if (!newlyBuiltTaxon.getParentName().equals(extantNode.getParentName())) {
				// One of these things is not like the other...
				System.out.println("Existing taxon " + extantNode.getFullName() + " should be under parent "
						+ newlyBuiltTaxon.getParentName() + " instead of " + extantNode.getParentName());

				try {
					TaxonNode newParent = fetchNode(footprint.get(0), newlyBuiltTaxon.getParentName());

					String xml = "<xml>\r\n<parent id=\"" + newParent.getId() + "\"/>\r\n";
					xml += "<child id=\"" + extantNode.getId() + "\"/>\r\n";
					xml += "</xml>";

					System.out.println("Doing a move " + xml);
					final String foo = xml;

					final NativeDocument ndoc = SISContainerApp.newNativeDocument(null);
					ndoc.post("/taxomatic/move", xml, new GenericCallback<String>() {
						public void onFailure(Throwable caught) {
							System.out.println("*** Failed lateral move ***\n" + foo);
						}
					...
					return fetchNode(extantNode.getId() + "");
				} catch (Exception e) {
					e.printStackTrace();
					System.exit(0);
					return null;
				...
				return extantNode; // Return the one with more info.
		} else {
			// I don't exist, so put me in...
			System.out.println("This taxon " + newlyBuiltTaxon.getFullName() + " doesn't exist yet.");
			parentNode.setAsParent(newlyBuiltTaxon);
			logNode(newlyBuiltTaxon);

			String curXML = TaxonNodeFactory.nodeToDetailedXML(newlyBuiltTaxon);

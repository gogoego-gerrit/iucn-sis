...
public class DEMImportTaxaUpdater implements Runnable {
	...
	public DEMImportTaxaUpdater(File source, Uniform uniform, String fileName, String user) {
		this.source = source;
	...
	public void buildTree() throws Exception {
		...
		for (Iterator<Row> iter = rowList.listIterator(); iter.hasNext();) {
			...
			String sis_code = null;

			try {
				sis_code = curCol.get("SIS_code").getString();
			} catch (Throwable e) {
				// Doesn't exist!
			}
			...
			if (kingdomN == null) {
				kingdomN = fetchNode(curKingdom, curKingdom, sis_code);
			...
			if (phylumN == null) {
				phylumN = fetchNode(curKingdom, curPhylum, sis_code);
			...
			if (classN == null) {
				classN = fetchNode(curKingdom, curClass, sis_code);
			...
			if (orderN == null) {
				orderN = fetchNode(curKingdom, curOrder, sis_code);
			...
			if (familyN == null) {
				familyN = fetchNode(curKingdom, curFamily, sis_code);
			...
			if (genusN == null) {
				genusN = fetchNode(curKingdom, curGenus, sis_code);
			...
			if (speciesN == null) {
				speciesN = fetchNode(curKingdom, curGenus + curSpecies, sis_code);
			...
			if (curRank != null) {
				...
				if (rankN == null) {
					rankN = fetchNode(curKingdom, curGenus + curSpecies + curRank, sis_code);

					if (rankN == null) {
						rankN = TaxonNodeFactory.createNode((long) newNodeID, curRank, TaxonNode.INFRARANK, "",
			...
			if (curSSP != null) {
				...
				if (sspN == null) {
					sspN = fetchNode(curKingdom, curGenus + curSpecies + curSSP, sis_code);
		...
		for (Object curEntry : tree.getKingdoms().values()) {
			...
			for (HashMap curLevel : curKingdom.getLevels()) {
				for (Iterator<TaxonNode> iter = curLevel.values().iterator(); iter.hasNext();) {
					...
					{
						System.out.println("Apparent there's a new taxon... " + cur.generateFullName());
						...
						System.out.println("Changed taxon " + cur.getId());

						Request request = new Request(Method.PUT , uriPrefix + "/browse/nodes/" + cur.getId(),
	...
	private TaxonNode fetchNode(String kingdomName, String fullNameNoSpaces, String sis_code) throws Exception {
		Request request = new Request(Method.GET , uriPrefix + "/browse/taxonName/" + kingdomName + "/"
		...
		if (response.getStatus().isSuccess()) {
			...
			if (sis_code != null) {
				...
				Request newRequest = new Request(Method.GET , uriPrefix + "/browse/nodes/" + sis_code + "/"
						+ URLEncoder.encode(fullNameNoSpaces.replaceAll("\\s", ""), "UTF-8"));
				Response newResponse = new Response(newRequest);
				uniform.handle(newRequest, newResponse);
	...
	public void run() {
		...
		try {
			...
			// "Failed converting assessments to SIS format.<br>");
			// statusMessage.append("Please report the following message:<br>");
			// statusMessage.append(DocumentUtils.getStackTraceAsString(e).
			// replaceAll("\n",
			// "<br>"));
			// failed.set(true);
			// }
			...
			// else
			// statusMessage.append(
			// "(Did not attempt conversion of assessments to SIS format due to a prior failure.)<br>"
			// );
			//
			// if( !failed.get() )
			// {
			...
			// else
			// statusMessage.append(
			// "(Did not save assessments in SIS due to a prior failure.)<br>");

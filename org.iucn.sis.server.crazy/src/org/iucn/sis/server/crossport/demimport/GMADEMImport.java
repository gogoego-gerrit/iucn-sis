 ...
 */
/**
public class GMADEMImport implements Runnable {
	...
	public GMADEMImport(File source, Uniform uniform, String fileName, String user) {
		this.source = source;
	...
	private void buildAssessments() throws Exception {
		log.append("<tr><td align=\"center\" colspan=\"10\">" + "<b><u>New Draft Assessments</u></b></td></tr>");
	...
	public void buildTree() throws Exception {
		...
		for (Iterator<Row> iter = rowList.listIterator(); iter.hasNext();) {
			...
			if (curRank != null) {
				...
				if (rankN == null) {
					rankN = fetchNode(curKingdom, curGenus + curSpecies + curRank);
	...
	private void exportNodes() throws Exception {
		log.append("<tr><td align=\"center\" colspan=\"10\">" + "<b><u>Newly Created Taxa</u></b></td></tr>");
		...
		for (Object curEntry : tree.getKingdoms().values()) {
			...
			for (HashMap curLevel : curKingdom.getLevels()) {
				for (Iterator<TaxonNode> iter = curLevel.values().iterator(); iter.hasNext();) {
					...
					} else if (cur.getStatus().equals("I")) {
						String curXML = TaxonNodeFactory.nodeToDetailedXML(cur);
	...
	private void redListingTableImport(Long curDEMid, AssessmentData curAssessment, HashMap<String, Object> data)
		...
		for (Iterator<Row> rows = rowSet.getSet().iterator(); rows.hasNext();) {
			...
			// Assessors
			data.put(CanonicalNames.RedListAssessors, createDataArray(curRow.get("Assessment_Authors").getString(
					Column.NEVER_NULL), true));
	...
	public void run() {
		...
		try {
			...
			log.append("<table border=\"1\"><tr>" + "<th>Level</th>" + "<th>SIS ID</th>" + "<th>Friendly Name</th>"
					+ "<th>Kingdom</th>" + "<th>Phylum</th>" + "<th>Class</th>" + "<th>Order</th>" + "<th>Family</th>"
					+ "<th>Genus</th>" + "<th>Species</th>" + "<th>Infrarank</th></tr>");
	...
	private void updateFootprint(ArrayList<String> footprint, TaxonNode taxon) {
		...
		// says
		if (!Arrays.deepEquals(footprint.toArray(), taxon.getFootprint()))
			System.out.println("Found disparate footprints for " + taxon.getId() + ". DEM: " + footprint.toString()
					+ " SIS: " + taxon.getFootprintAsString());

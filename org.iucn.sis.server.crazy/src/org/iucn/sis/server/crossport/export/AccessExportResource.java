	...
	public void acceptRepresentation(Representation entity) throws ResourceException {
		...
		if (!AccessExport.isRunning()) {
			...
			if (id != null && !id.equals("")) {
				...
				new Thread(new AccessExport(getContext().getClientDispatcher(), id)).start();
			} else
				new Thread(new AccessExport(getContext().getClientDispatcher())).start();
		}
	...
	public Representation represent(final Variant variant) {
		...
		if (AccessExport.isRunning()) {
			...
			if (workingsetid != null) {
				...
				mdbFile = SISBootstrap.getSISInstance() + ".mdb";
				zipFile = SISBootstrap.getSISInstance() + ".zip";
			}
			sb

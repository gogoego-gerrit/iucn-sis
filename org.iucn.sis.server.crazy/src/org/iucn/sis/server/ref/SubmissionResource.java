		...
		try {
			...
			for (int i = 0; i < references.getLength(); i++) {
				...
				for (int j = 0; j < fields.getLength(); j++) {
					...
					if (c != null) {
						SysDebugger.getInstance().println("  SET: " + name + " = " + value);
				...
				if (id == null || !newid.equals(id)) {
					...
					SysDebugger.getInstance().println("Inserting new reference " + newid);
					...
					try {
						...
						dbx.printStackTrace();
						SysDebugger.getInstance().println("Apparently that reference already exists");
					...
					SysDebugger.getInstance().println("Existing reference " + id + " not materially changed");
				}

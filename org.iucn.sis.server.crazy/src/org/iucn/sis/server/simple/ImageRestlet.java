...
import org.iucn.sis.client.components.panels.imagemanagement.ManagedImageData;
...
public class ImageRestlet extends ServiceRestlet {
	...
	private void handleGet(Request request, Response response, String taxonId) {
		try {
			...
			SysDebugger.getInstance().println(FilenameStriper.getIDAsStripedPath(taxonId));
	...
	private void handlePost(Request request, Response response, String taxonId) {
		...
		try {
			...
			for (int i = 0; i < list.size() && file == null; i++) {
				...
				SysDebugger.getInstance().println(encoding);
			...
			if (file == null) {
				SysDebugger.getInstance().println("file null");
	...
	private void handleBatchUpload(int id, Response response){
		...
		try{
			...
			ZipFile zip = new ZipFile(tempFile, "Cp1252");
			Enumeration entries;

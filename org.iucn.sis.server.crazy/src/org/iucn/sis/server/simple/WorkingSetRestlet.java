...
public class WorkingSetRestlet extends ServiceRestlet {
	...
	protected final WorkingSetRestletWorker worker;
	...
	public WorkingSetRestlet(String vfsroot, Context context) {
		...
		worker = new WorkingSetRestletWorker(vfs);
		try {
	...
	public void definePaths() {
		...
		paths.add("/workingSet/unsubscribe/{username}/{id}");
	...
	private void deleteFromWorkingSet(Request request, Response response, String mode) {
		...
		if (mode == null) { //FIXME: Make this clear. mode == null means unsubscribe is mediocre.
			response.setStatus(doUnsubscribe("public", url, id));
		} else if (mode.equals(WorkingSetData.PRIVATE)) {
			String workingSetURL = getPrivateWorkingSetURL(username, id);
			VFSPath privateWorkingSetPath = null;
			try {
			...
			if (locked) {
				if( WorkingSetIO.deleteWorkingSet(vfs, id) )
	...
	private Status doUnsubscribe(String mode, final String url, final String id) {
		boolean locked = FileLocker.impl.aquireWithRetry(url, MAXTRIESTOWRITE);
		...
		if (vfs.exists(userUrl)) {
			...
			for (String publicWorkingSet : publicWorkingSets) {
				...
				xml.append("<workingSet creator=\"" + element.getAttribute("creator") + "\" id=\""
						...
						+ XMLUtils.clean(info.getElementsByTagName("name").item(0).getTextContent()) + "\"/>");
			}
...
//			String taxa = getTaxaString(doc);
			WorkingSetData ws = WorkingSetIO.readPublicWorkingSetAsWorkingSetData(vfs, id);
			if( ws == null )
				WorkingSetIO.readPrivateWorkingSetAsWorkingSetData(vfs, id, username);
			String taxa = ws.getSpeciesIDsAsString();
			
			Request newRequest = new Request(Method.GET, "riap://application/browse/footprint/null");
...
//				boolean allExisted = true;
				...
				if (vfs.exists(workingSetURL)) {
					...
					//TODO: FIXME: move calls like this to SISContainerApp, make it DBProvidingApplication
					final ExecutionContext ec = SISContainerApp.amIOnline ? PersistentLockRepository.getExecutionContext() : null;
					...
					for (int i = 0; i < publicNodes.getLength(); i++) {
						...
						if (vfs.exists(getPublicWorkingSetURL(publicID))) {
							...
							if( ec != null ) {
								...
								query.select("workflow_status", "status");
								query.constrain(new CanonicalColumnName("workflow_status", "workingsetid"), QConstraint.CT_EQUALS, publicID);
								...
								if (rl.getRow() != null) {
									workingSet.setAttribute("workflow_status", rl.getRow().get("status").toString());
								}
							}
							
							publicWorkingSetElement.replaceChild(workingSet, tempPublic);
						} else {
							publicWorkingSetElement.removeChild(tempPublic);
							
							String url = getWorkingSetFileURL(username);
							doUnsubscribe("public", url, publicID);
						}
					}
...
//					response.setStatus(Status.SUCCESS_OK);
	...
	public void performService(Request request, Response response) {
		...
		if (request.getMethod().equals(Method.GET)) {
			...
			if (request.getResourceRef().getPath().startsWith("/workingSet/unsubscribe/"))
				deleteFromWorkingSet(request, response, null);
			else if (request.getResourceRef().getPath().startsWith("/workingSet/private/"))
	...
	private void subscribeToPublicWorkingSet(Request request, Response response) {
		...
		try {
			response.setEntity(new DomRepresentation(MediaType.TEXT_XML, worker.subscribeToPublicWorkingSet(id, username)));
			response.setStatus(Status.SUCCESS_OK);

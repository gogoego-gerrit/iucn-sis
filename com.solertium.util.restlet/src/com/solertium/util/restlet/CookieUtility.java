 ...
 * Solertium Corporation, October, 2008
 * 
...
public class CookieUtility {
	 ...
	 */
	public static String associateHTTPBrowserID(final Request request,
			final Response response) {
		final Cookie existingID = request.getCookies().getFirst(
				NAME_HTTP_BROWSER_ID);
		if (existingID == null) {
			final CookieSetting newID = CookieUtility.createHTTPBrowserID();
			request.getCookies().add(newID); // so subsequent requests find it
			if (response != null)
	...
	public static CookieSetting createHTTPBrowserID() {
		...
		cs.setAccessRestricted(true); // requires > Restlet 1.1M1
		return cs;
	}
	 ...
	 * @return HTTPBrowserID cookie value
	 */
	public static String resetHTTPBrowserID(final Request request,
			final Response response) {

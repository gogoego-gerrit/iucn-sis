...
public abstract class QBTreeItem extends VerticalPanel {
	...
	protected LayoutContainer childPanel;			//Displays children
	...
	protected LayoutContainer leftMarginPanel;		//Indentations
	...
	public QBTreeItem(int leftMargin, boolean isRoot) {
		...
		childPanel = new LayoutContainer();
	...
	private Widget buildLeftMargin() {
		if (leftMarginPanel == null) {
			leftMarginPanel = new LayoutContainer();
			leftMarginPanel.setWidth(leftMargin);
		}
	...
	public ArrayList<QBTreeItem> getChildren() {
		return children;
	}
	...
	class RowUI extends HorizontalPanel {
		...
		public boolean add(final Widget w) {
			if (w instanceof StyledHTML) {
				((StyledHTML) w).addClickListener(new ClickListener() {
					public void onClick(Widget sender) {
						final Menu menu = getMyContextMenu();
						if (menu != null)
							menu.show(w);
					}
				});
	...
	class FieldRootLevelContextMenu extends Menu {

		public FieldRootLevelContextMenu() {
			super();
			
			final MenuItem item = new MenuItem("Add Field", new SelectionListener<MenuEvent>() {

package org.iucn.sis.client.panels.workflow;
...
// setAlignment(HorizontalAlignment.CENTER);
	...
	private void updateCommentPanel(final DrawsLazily.DoneDrawingCallback callback) {
		...
		document.get(UriBase.getInstance().getWorkflowBase() + "/workflow/set/" + workingSetID + "/notes/" + scope, new GenericCallback<String>() {
	...
	private void draw(final DrawsLazily.DoneDrawingCallback callback) {
		final NativeDocument document = SimpleSISClient.getHttpBasicNativeDocument();
		document.get(UriBase.getInstance().getWorkflowBase() + "/workflow/set/"+workingSetID+"/status", new GenericCallback<String>() {
			public void onSuccess(String result) {
				final RowParser parser = new RowParser(document);
				if (parser.getRowCount() == 0)
		...
		addButton(new Button("Add Comment", new SelectionListener<ButtonEvent>() {
			public void componentSelected(ButtonEvent ce) {
				if (area.getValue() != null && !"".equals(area.getValue())) {
					...
					final NativeDocument document = SimpleSISClient.getHttpBasicNativeDocument();
					document.post(UriBase.getInstance().getWorkflowBase() + "/workflow/set/" + workingSetID + "/status", bodyBuilder.toString(), new GenericCallback<String>() {
						public void onSuccess(String result) {

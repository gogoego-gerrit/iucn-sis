...
public class WorkingSetFullPanel extends ContentPanel {
	...
	private void jumpTo() {
		...
		if (taxonNameToTaxonID.containsKey(name)) {
			TaxonomyCache.impl.fetchTaxon(taxonNameToTaxonID.get(name), true, new GenericCallback<Taxon >() {
	...
	public void newWorkingSet() {
		...
		currentWorkingSet = new WorkingSet();
	...
	private void refreshDelayed() {
		...
		if (currentWorkingSet == null) {
			...
			grid.setHTML(0, 1, currentWorkingSet.getCreatorUsername());
			grid.setHTML(1, 1, FormattedDate.impl.getDate(currentWorkingSet.getCreatedDate()));
	...
	private void saveWorkingSet(final boolean saveAndExit) {
		...
		if (saveNeeded) {
			...
			currentWorkingSet.setName(XMLUtils.clean(name));

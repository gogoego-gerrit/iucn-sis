 ...
 */
public class LegacyClassificationScheme extends Display {
	...
	public class ClassificationSchemeEntry extends BaseModel implements Referenceable {
		...
		private DisplayStructure<?, ?> struct;
		private Object key;
		private ArrayList<String> pretty;
		
		public ClassificationSchemeEntry(String key, DisplayStructure<?, ?> struct) {
	 ...
	 */
	private HashMap<String, DisplayStructure> selected = null;
	...
	public LegacyClassificationScheme(TreeData displayData) {
		super(displayData);

		treeData = displayData;
		selected = new HashMap<String, DisplayStructure>();
		...
		for (Iterator<TreeDataRow> iter = treeData.getTreeRoots().listIterator(); iter.hasNext();)
			buildMaps(iter.next(), null, "");
	...
	private void buildMaps(TreeDataRow curRow, TreeDataRow parent, String parentDesc) {
		...
		for (Iterator<TreeDataRow> iter = curRow.getChildren().listIterator(); iter.hasNext();)
			buildMaps(iter.next(), curRow, levelID.equals("0") ? "" : parentDesc + description + " -> ");
	}
			...
			public void componentSelected(ButtonEvent ce) {
				...
				Button ok = new Button("OK", new SelectionListener<ButtonEvent>() {
					public void componentSelected(ButtonEvent ce) {
						...
						(selected.get(clsch.getKey())).setId((String)clsch.getKey());
		...
		for (int i = 0; i < entries.length; i++) {
			...
			try {
				pagingLoader.getFullList().add(new ClassificationSchemeEntry(
					(String)curEntry.getKey(), (DisplayStructure)curEntry.getValue()
				));
...
//		};
	...
	public HashMap<String, String> getCodeToDesc() {
		return codeToDesc;
	}

	public HashMap<String, String> getCodeToSelectedDesc() {
		return codeToSelectedDesc;
	}

	public HashMap<String, String> getCodeToLevelID() {
		return codeToLevelID;
	}

	public HashMap<String, DisplayStructure> getSelected() {
		return selected;
	}
	...
	public void save() {
		for( Entry<String, DisplayStructure> curSelected : selected.entrySet() ) {
			...
			curSelected.getValue().save(field, subfield);
		}
	}
	...
	public void setData(Field field) {
		this.field = field;
		if( field != null ) {
			//FIXME: This should populate something!
		} else {
			selected.clear();
		}
	}
	 ...
	 */
	public void setSelectedAsStructures(Map<String, DisplayStructure> newData) {
		selected.clear();

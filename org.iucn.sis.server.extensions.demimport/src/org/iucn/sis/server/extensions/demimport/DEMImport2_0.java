...
import org.iucn.sis.shared.api.structures.SISThreatStructure;
 ...
 * TODO: FINISH THIS!
 * 
 * This will perform an import of DEM data, directly using the SIS 2.0 models.
 * The DEMImport.java implementation uses the old AssessmentData and TaxonNode objects
 * as a pass-through, then uses the Conversion* classes to migrate them to the
 * SIS 2.0 models. 
 * 
 * @author adam.schwartz
 */
public class DEMImport2_0 implements Runnable {
...
//
//	private LinkedHashMap<Long, Taxon> assessedNodesBySpc_id;
//	private LinkedHashMap<String, Long> spcNameToIDMap;
//	private LinkedHashMap<Long, Assessment> assessments;
//
...
//
//	public DEMImport2_0(File source, Context context, String fileName, String user, String secret) {
//		this.source = source;
...
//		assessments = new LinkedHashMap<Long, Assessment>();
...
//
//				CommonName commonName = new CommonName(name, language, isoCode);
...
//				curSyn.setInfraType(Infratype.getInfratype(infraType));
...
//					curSyn.setNotes(new HashSet<Notes>());
//					curSyn.getNotes().add(note);
//				}
//
//				changed = curNode.getSynonyms().add(curSyn);
//			}
//		}
//	}
//
...
//			
//			List<Assessment> drafts = SIS.get().getAssessmentIO().readDraftAssessmentsForTaxon(Integer.valueOf(curNode.getId()));
...
//					failed.set(true);
//					return;
//				} else if( !newIsGlobal && cur.isRegionConflict(curAssessment) != null ) {
...
//			riversTableImport(curDEMid, curAssessment);
//			lakesTableImport(curDEMid, curAssessment);
...
//				curSSP = curSSP.trim();
//
//			kingdomN = tree.getNode(TaxonLevel.KINGDOM, curKingdom, curKingdom);
//			if (kingdomN == null) {
//				kingdomN = fetchNode(curKingdom, curKingdom);
//
//				if (kingdomN == null) {
//					kingdomN = createTaxon(newNodeID, curKingdom, TaxonLevel.KINGDOM);
//					kingdomN.setFriendlyName(kingdomN.getName());
//
...
//			nodes.add(kingdomN);
//
//			phylumN = tree.getNode(TaxonLevel.PHYLUM, curKingdom, curPhylum);
//			if (phylumN == null) {
//				phylumN = fetchNode(curKingdom, curPhylum);
//
//				if (phylumN == null) {
//					phylumN = createTaxon(newNodeID, curPhylum, TaxonLevel.PHYLUM);
//					phylumN.setFriendlyName(phylumN.getName());
//
//					phylumN.setFootprint(footprint.toArray(new String[TaxonLevel.PHYLUM]));
//				}
//
//				phylumN.setParent(kingdomN);
...
//			nodes.add(phylumN);
//
//			classN = tree.getNode(TaxonLevel.CLASS, curKingdom, curClass);
//			if (classN == null) {
//				classN = fetchNode(curKingdom, curClass);
//
//				if (classN == null) {
//					classN = createTaxon(newNodeID, curClass, TaxonLevel.CLASS);
//					classN.setFriendlyName(classN.getName());
//
//					classN.setFootprint(footprint.toArray(new String[TaxonLevel.CLASS]));
//				}
//
//				classN.setParent(phylumN);
//				tree.addNode(curKingdom, classN);
//			}
//			updateFootprint(footprint, classN);
//			nodes.add(classN);
//
//			orderN = tree.getNode(TaxonLevel.ORDER, curKingdom, curOrder);
//			if (orderN == null) {
//				orderN = fetchNode(curKingdom, curOrder);
//
//				if (orderN == null) {
//					orderN = createTaxon(newNodeID, curOrder, TaxonLevel.ORDER);
//
//					orderN.setFriendlyName(orderN.getName());
//
//					orderN.setFootprint(footprint.toArray(new String[TaxonLevel.ORDER]));
//				}
//
//				orderN.setParent(classN);
//				tree.addNode(curKingdom, orderN);
//			}
//			updateFootprint(footprint, orderN);
//			nodes.add(orderN);
//
//			familyN = tree.getNode(TaxonLevel.FAMILY, curKingdom, curFamily);
//			if (familyN == null) {
//				familyN = fetchNode(curKingdom, curFamily);
//
//				if (familyN == null) {
//					familyN = createTaxon(newNodeID, curFamily, TaxonLevel.FAMILY);
//					familyN.setFriendlyName(familyN.getName());
//
//					familyN.setFootprint(footprint.toArray(new String[TaxonLevel.FAMILY]));
//				}
//
//				familyN.setParent(orderN);
//				tree.addNode(curKingdom, familyN);
//			}
//			updateFootprint(footprint, familyN);
//			nodes.add(familyN);
//
//			genusN = tree.getNode(TaxonLevel.GENUS, curKingdom, curGenus);
//			if (genusN == null) {
//				genusN = fetchNode(curKingdom, curGenus);
//
//				if (genusN == null) {
//					genusN = createTaxon(newNodeID, curGenus, TaxonLevel.GENUS);
//					genusN.setFriendlyName(genusN.getName());
//
//					genusN.setFootprint(footprint.toArray(new String[TaxonLevel.GENUS]));
//				}
//
//				genusN.setParent(familyN);
//				tree.addNode(curKingdom, genusN);
//			}
//			updateFootprint(footprint, genusN);
//			nodes.add(genusN);
//
//			speciesN = tree.getNode(TaxonLevel.SPECIES, curKingdom, curGenus + " " + curSpecies);
//			if (speciesN == null) {
//				speciesN = fetchNode(curKingdom, curGenus + curSpecies);
//
//				if (speciesN == null) {
//					speciesN = createTaxon(newNodeID, curSpecies, TaxonLevel.SPECIES);
//
//					speciesN.setFriendlyName(curGenus + " " + speciesN.getName());
//					spcNameToIDMap.put(speciesN.getFullName(), new Long(specID));
//
//					speciesN.setFootprint(footprint.toArray(new String[TaxonLevel.SPECIES]));
//				}
//
//				speciesN.setFriendlyName(curGenus + " " + speciesN.getName());
//				speciesN.setTaxonomicAuthority(spcTaxonomicAuthority);
//
//				speciesN.setParent(genusN);
...
//				rankN = tree.getNode(TaxonLevel.INFRARANK, curKingdom, curGenus + " " + curSpecies + " " + curInfratype
//						+ " " + curInfraEpithet);
//				if (rankN == null) {
//					rankN = fetchNode(curKingdom, curGenus + curSpecies + curInfratype + curInfraEpithet);
//
//					if (rankN == null) {
//						rankN = createTaxon(newNodeID, curInfraEpithet, TaxonLevel.INFRARANK);
//
//						rankN.setFootprint(footprint.toArray(new String[TaxonLevel.INFRARANK]));
//
//						if (curInfratype.trim().matches("^ssp\\.?$"))
//							rankN.setInfratype(Infratype.getInfratype(Infratype.INFRARANK_TYPE_SUBSPECIES, rankN));
//						else if (curInfratype.trim().matches("^var\\.?$")
//								&& rankN.getFootprint()[0].equalsIgnoreCase("PLANTAE"))
//							rankN.setInfratype(Infratype.getInfratype(Infratype.INFRARANK_TYPE_VARIETY, rankN));
//						else {
//							failed.set(true);
...
//				if (curInfratype == null)
//					sspN = tree
//							.getNode(TaxonLevel.SUBPOPULATION, curKingdom, curGenus + " " + curSpecies + " " + curSSP);
//				else
//					sspN = tree.getNode(TaxonLevel.INFRARANK_SUBPOPULATION, curKingdom, curGenus + " " + curSpecies
...
//
//					if (sspN == null) {
//						sspN = createTaxon(newNodeID, curSSP, TaxonLevel.SUBPOPULATION);
...
//	}
//
//	private void countryTableImport(Long curDEMid, Assessment curAssessment)
//			throws Exception {
//		SelectQuery select = new SelectQuery();
...
//	}
//
//	private Taxon createTaxon(int id, String name, int level) {
//		Taxon newTaxon = new Taxon();
//		newTaxon.setId(id);
//		newTaxon.setName(name);
//		newTaxon.setTaxonLevel(TaxonLevel.getTaxonLevel(level));
//		newTaxon.setTaxonStatus(TaxonStatus.fromCode(STATUS_NEW));
//		return newTaxon;
//	}
//	
//	private void distributionTableImport(Long curDEMid, Assessment curAssessment)
//			throws DBException {
//		SelectQuery select = new SelectQuery();
...
//
//				sendNote(curAssessment.getType(), message, curAssessment.getId(), CanonicalNames.MapStatus);
//			} catch (Exception e) {
...
//	}
//
//	private void ecosystemServicesTableImport(Long curDEMid, Assessment curAssessment)
//			throws DBException {
//		SelectQuery select = new SelectQuery();
...
//		for (Iterator<Assessment> iter = assessments.values().iterator(); iter.hasNext();) {
...
//
//			Request request = new Request(Method.PUT, url.toString(), new StringRepresentation(xml, MediaType.TEXT_XML, null,
//					CharacterSet.UTF_8));
//			request.setChallengeResponse(new ChallengeResponse(ChallengeScheme.HTTP_BASIC, user, secret));
//			Response response = context.getClientDispatcher().handle(request);
//			
//			if (!response.getStatus().isSuccess())
//				System.out.println("Error putting draft assessment for  " + assessment.getSpeciesName());
//			else
...
//		
//		Request request = new Request(Method.PUT, uriPrefix + "/workingSet/public/" + user, new StringRepresentation(
...
//				for (Iterator<Taxon> iter = curLevel.values().iterator(); iter.hasNext();) {
//					Taxon cur = iter.next();
//
...
//						logNode(cur);
//
//						String curXML = TaxonFactory.nodeToDetailedXML(cur);
//					
//						Request request = new Request(Method.PUT, "riap://application/taxomatic/new", new StringRepresentation(curXML,
...
//						String curXML = TaxonFactory.nodeToDetailedXML(cur);
//
//						if( TaxonIO.writeTaxon(cur, SIS.get().getVFS()) == null ) {
//							failed.set(true);
...
//	}
//
//	private void faoMarineTableImport(Long curDEMid, Assessment curAssessment)
//			throws Exception {
//		SelectQuery select = new SelectQuery();
//		select.select("coding_occurence", "*");
//		select.constrain(new CanonicalColumnName("coding_occurence", "Sp_code"), QConstraint.CT_EQUALS, curDEMid);
...
//	}
//
//	private Taxon fetchNode(String kingdomName, String fullNameNoSpaces) throws Exception {
//		String id = TaxonomyDocUtils.getIDByName(kingdomName, fullNameNoSpaces.replaceAll("\\s", ""));
//		
//		if (id != null) {
//			return TaxonIO.readNode(id, SIS.get().getVFS());
//		} else
//			return null;
//	}
//
...
//	}
//
//	private void growthFormTableImport(Long curDEMid, Assessment curAssessment)
//			throws DBException {
//		SelectQuery select = new SelectQuery();
...
//	}
//
//	private void habitatTableImport(Long curDEMid, Assessment curAssessment)
//			throws DBException {
//		SelectQuery select = new SelectQuery();
...
//	}
//
//	private void lakesTableImport(Long curDEMid, Assessment curAssessment)
//			throws DBException {
//		SelectQuery select = new SelectQuery();
...
//	}
//
//	private void landCoverTableImport(Long curDEMid, Assessment curAssessment)
//			throws DBException {
//		SelectQuery select = new SelectQuery();
...
//	}
//
//	private void lifeHistoryTableImport(Long curDEMid, Assessment curAssessment)
//			throws DBException {
//		SelectQuery select = new SelectQuery();
...
//	}
//
//	private void livelihoodsTableImport(Long curDEMid, Assessment curAssessment)
//			throws DBException {
//		SelectQuery select = new SelectQuery();
...
//	}
//
//	private void populationTableImport(Long curDEMid, Assessment curAssessment)
//			throws DBException {
//		SelectQuery select = new SelectQuery();
...
//	private void redListingTableImport(Long curDEMid, Assessment curAssessment)
//			throws DBException {
//		SelectQuery select = new SelectQuery();
//		select = new SelectQuery();
...
//	 */
//
//	private void riversTableImport(Long curDEMid, Assessment curAssessment)
//			throws DBException {
//		SelectQuery select = new SelectQuery();
...
//
//	private void sendNote(String assessmentType, String noteBody, int assessmentID, Field field) {
//		Notes note = new Notes();
//		note.setValue(noteBody);
//		note.setField(field);
...
//	}
//
//	private void systematicsTableImport(Long curDEMid, Assessment curAssessment)
//			throws DBException {
//		SelectQuery select = new SelectQuery();
//		select.select("Systematics", "*");
...
//	}
//
//	private void threatTableImport(Long curDEMid, Assessment curAssessment)
//			throws DBException {
//		SelectQuery select = new SelectQuery();
...
//	 */
//	@SuppressWarnings(value = "unchecked")
//	private void useTradeImport(Long curDEMid, Assessment curAssessment)
//			throws DBException {
//		SelectQuery select = new SelectQuery();
...
//	}
//
//	private void utilisationTableImport(Long curDEMid, Assessment curAssessment)
//			throws DBException {
//		SelectQuery select = new SelectQuery();

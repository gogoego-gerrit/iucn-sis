...
public class PublicationRestlet extends BaseServiceRestlet {
	...
	public void handlePut(Representation entity, Request request,
		...
		if (request.getResourceRef().getPath().endsWith("targets"))
			addTarget(entity, response, session);
		else
			submitData(entity, request, response, session);
	}
	...
	private void submitData(Representation entity, Request request, Response response, Session session) throws ResourceException {
		final PublicationIO io = new PublicationIO(session);
	...
	private void addTarget(Representation entity, Response response, Session session) throws ResourceException {
